...
| varB : id -> var (* locally-bound variable, by a dependent function type *)
| varR : id -> var (* locally-bound variable, by a recursive binder *)
.
...
| tbot_wf: forall n,
    ty_wf n TBot
| tall_wf: forall n t1 t2,
    ty_wf 0 t1 -> (* ensures strictly positive types *)
    ty_wf n t2 ->
    ty_wf n (TAll t1 t2)
| tsel_wf_r: forall n x,
    n > x ->
    ty_wf n (TSel (varR x))
| tsel_wf_b: forall n x,
    ty_wf n (TSel (varB x))
| tsel_wf_f: forall n x,
    ty_wf n (TSel (varF x))
| tsel_wf_h: forall n x,
    ty_wf n (TSel (varH x))
| tmem_wf: forall n t1 t2,
    ty_wf n t1 ->
    ty_wf n t2 ->
    ty_wf n (TMem t1 t2)
| tbind_wf: forall n t,
...
Inductive closed: nat(*R*) -> nat(*B*) -> nat(*H*) -> nat(*F*) -> ty -> Prop :=
| cl_top: forall r i j k,
    closed r i j k TTop
| cl_bot: forall r i j k,
    closed r i j k TBot
| cl_all: forall r i j k T1 T2,
    closed 0  i j k T1 -> (* due to strict positivity *)
    closed r (S i) j k T2 ->
    closed r i j k (TAll T1 T2)
| cl_sel: forall r i j k x,
    k > x ->
    closed r i j k (TSel (varF x))
| cl_selh: forall r i j k x,
    j > x ->
    closed r i j k (TSel (varH x))
| cl_selb: forall r i j k x,
    i > x ->
    closed r i j k (TSel (varB x))
| cl_selr: forall r i j k x,
    r > x ->
    closed r i j k (TSel (varR x))
| cl_mem: forall r i j k T1 T2,
    closed r i j k T1 ->
    closed r i j k T2 ->
    closed r i j k (TMem T1 T2)
| cl_bind: forall r i j k T,
    closed (S r) i j k T ->
    closed r i j k (TBind T)
| cl_and: forall r i j k T1 T2,
    closed r i j k T1 ->
    closed r i j k T2 ->
    closed r i j k (TAnd T1 T2)
.
...
Fixpoint open_rec (k: nat) (u: var) (T: ty) { struct T }: ty :=
  match T with
    ...
    | TBind T => TBind (open_rec k u T)
    | TAnd T1 T2 => TAnd (open_rec k u T1) (open_rec k u T2)
  end.

Definition open u T := open_rec 0 u T.

(* substitute u for all occurrences of (varR k) *)
Fixpoint open_rec_r (k: nat) (u: var) (T: ty) { struct T }: ty :=
  match T with
    | TTop        => TTop
    | TBot        => TBot
    | TAll T1 T2  => TAll T1(*since strictly positive*) (open_rec_r k u T2)
    | TSel (varF x) => TSel (varF x)
    | TSel (varH i) => TSel (varH i)
    | TSel (varB i) => TSel (varB i)
    | TSel (varR i) => if beq_nat k i then TSel u else TSel (varR i)
    | TMem T1 T2  => TMem (open_rec_r k u T1) (open_rec_r k u T2)
    | TBind T => TBind (open_rec_r (S k) u T)
    | TAnd T1 T2 => TAnd (open_rec_r k u T1) (open_rec_r k u T2)
  end.
...
Fixpoint vset n := match n with
                     | 0 => vl -> Prop (* TODO: can we enforce that 0, the Prop is True?  *)
                     | S n => vset n -> vl -> Prop
                   end.
...
Definition vseta_pred (P: forall n, vl -> vset n -> Prop): vseta :=
  fun n =>
    match n with
    | 0 =>   fun v => True
    | S n => fun vs v => P n v vs
    end.

(* \lBrace \rBrace *)
Notation "⦃ v vs n | P ⦄" := (vseta_pred (fun n v vs => P))
...
(* Check ⦃ v vs n | exists H T t, v = (vabs H T t) /\ n > 0 /\ vs ⊑# (vseta_top n) ⦄.  *)
...
(* If we define the type interpretation function such that it returns vseta values directly, e.g.,

   Program Fixpoint ty_vset (env: list vseta) (GH: list vseta) (T:ty) {measure (tsize_flat T)}: vseta

   then we run into issues with recursive calls in the body of ty_vset, i.e., Coq won't be able
   to implicitly provide the evidence that the supplied argument is smaller (it's unclear why).

   However, we can circumvent the issues by wrapping the return type in Dom: *)
Inductive Dom: Type :=
| DomV: vseta -> Dom.
(* so that the signature becomes

  Program Fixpoint ty_vset (env: list vseta) (GH: list vseta) (T:ty) {measure (tsize_flat T)}: Dom  *)

(* The price we pay is explicit (un)wrapping of vseta in Dom *)
Definition unDom (d: Dom): vseta :=
  match d with
  | DomV vs => vs
  end.
(* which we can conveniently hide in lifted versions of the vseta ops on Dom.
  The '*' symbol indicates which argument is enclosed in Dom: *)
Notation "p    ⋵* vs"  := (vseta_mem (fst p) (snd p) (unDom vs)) (at level 79).
Notation "vs1 *⊑  vs2" := ((unDom vs1) ⊑ vs2) (at level 75).
Notation "vs1  ⊑* vs2" := (vs1 ⊑ (unDom vs2)) (at level 75).
Notation "vs1 *⊑* vs2" := ((unDom vs1) ⊑ (unDom vs2)) (at level 75).
Notation "vs1 *⊓* vs2" := (DomV (unDom vs1 ⊓ unDom vs2)) (at level 65, right associativity).
Notation "vs1 *⊔* vs2" := (DomV (unDom vs1 ⊔ unDom vs2)) (at level 70, right associativity).
Notation "*⨆*⦃ vs | P ⦄" := (DomV (vseta_big_join (fun vs => P))) (vs at level 99).
Notation "*⨅*⦃ vs | P ⦄" := (DomV (vseta_big_meet (fun vs => P))) (vs at level 99).
...
Program Fixpoint ty_vset (env: list vseta) (GH: list vseta) (T:ty) {measure (tsize_flat T)}: Dom :=
  ...
  | TAll T1 T2 =>
    (* TODO: do we really need to include this condition here?
    closed 0 0 (length GH) (length env) T1 /\ closed 0 1 (length GH) (length env) T2 /\ *)
    let T2x := (open (varH (length GH)) T2) in
    DomV ⦃ v vs n | exists H t, v = (vabs H T1 t)
                      /\ forall vx vsx, (vx,vsx) ⋵* (ty_vset env GH T1) ->
                                  exists vd u, tevaln (vx :: H) t u /\
                                          (u, vd) ⋵* (ty_vset env (vd :: GH) T2x) ⦄

  | TMem T1 T2 =>
    DomV ⦃ v vs n | exists H T, v = (vty H T)
                           /\ ((unDom (ty_vset env GH T1)) n) ⊑# vs
                           /\ vs ⊑# ((unDom (ty_vset env GH T2)) n) ⦄
  ...
  | TBind T =>
    (* TODO needed here? closed 1 0 (length GH) (length env) T  *)
    let Tx := (open_r (varH (length GH)) T) in
     *⨅*⦃ vs | (ty_vset env (vs :: GH) Tx) *⊑ vs ⦄ (* choose LFP for inductive types *)

    (* GFP would be:
     *⨆*⦃ vs | vs ⊑* (ty_vset env (vs :: GH) Tx) ⦄) *)

  | TAnd T1 T2 =>
    (ty_vset env GH T1) *⊓* (ty_vset env GH T2)
...
Next Obligation. simpl. omega. Qed.
Next Obligation. simpl. omega. Qed.
Next Obligation. simpl. unfold open_r. rewrite <-open_r_preserves_size. omega. Qed. (* TBind case: open_r *)
Next Obligation. simpl. omega. Qed.
Next Obligation. simpl. omega. Qed.
...
Definition R_env venv genv tenv := (* TODO: include well-formedness here? *)
...
Lemma closed_splice: forall h i j k T n,
  closed h i j k T ->
  closed h i (S j) k (splice n T).
Proof.
...
Lemma closed_inc_mult: forall h i j k T,
  closed h i j k T ->
  forall h' i' j' k',
  h' >= h -> i' >= i -> j' >= j -> k' >= k ->
  closed h' i' j' k' T.
Proof.
  intros h i j k T H. induction H; intros; eauto; try solve [constructor; omega].
  - apply cl_all. apply IHclosed1; omega. apply IHclosed2; omega.
  - constructor. apply IHclosed; omega.
Qed.

Lemma closed_inc: forall h i j k T,
  closed h i j k T ->
  closed h i (S j) k T.
Proof.
  intros. apply (closed_inc_mult h i j k T H h i (S j) k); omega.
Qed.

Lemma closed_splice_idem: forall h i j k T n,
                            closed h i j k T ->
                            n >= j ->
                            splice n T = T.
Proof.
...
Lemma stp_closed : forall G GH T1 T2,
                     stp G GH T1 T2 ->
                     closed 0 0 (length GH) (length G) T1 /\ closed 0 0 (length GH) (length G) T2.
Proof.
  intros. induction H;
...
Lemma closed_upgrade: forall h i j k i' T,
 closed h i j k T ->
 i' >= i ->
 closed h i' j k T.
Proof.
 intros. apply (closed_inc_mult h i j k T H h i' j k); omega.
Qed.

Lemma closed_upgrade_free: forall h i j k j' T,
 closed h i j k T ->
 j' >= j ->
 closed h i j' k T.
Proof.
...
Lemma closed_upgrade_freef: forall h i j k k' T,
 closed h i j k T ->
 k' >= k ->
 closed h i j k' T.
Proof.
...
Lemma closed_upgrade_freer: forall h h' i j k  T,
 closed h i j k T ->
 h' >= h ->
 closed h' i j k T.
Proof.
...
Lemma closed_open: forall h i j k V T, closed h (i+1) j k T -> closed 0 i j k (TSel V) ->
  closed h i j k (open_rec i V T).
Proof.
  intros.
  generalize dependent i. generalize dependent h.
  induction T; intros; inversion H; simpl; auto; try constructor;
    try eapply IHT1; auto; try apply IHT2; auto.
  - inversion H0; subst; simpl; eauto.
  - (* Case "TVarB". *) simpl.
    case_eq (beq_nat i x); intros E. eauto.
    eapply closed_upgrade_freer. eauto. omega.
    subst. constructor. apply beq_nat_false_iff in E. omega.
Qed.

Lemma closed_open_r: forall h i j k V T, closed (h + 1) i j k T -> closed h i j k (TSel V) ->
  closed h i j k (open_rec_r h V T).
Proof.
  intros.
  generalize dependent h. generalize dependent i.
  induction T; intros; inversion H; simpl; auto; try constructor;
    try eapply IHT1; auto; try apply IHT2; auto.
  -  eapply closed_upgrade. eauto. omega.
  - (* Case "TVarB". *) simpl.
    case_eq (beq_nat h x); intros E. eauto.
 apply beq_nat_false_iff in E. subst. constructor. omega.
  - (* TBind *) apply IHT; auto. eapply closed_upgrade_freer; eauto.
Qed.
...
Lemma stp_refl_aux: forall n T G GH,
  closed 0 0 (length GH) (length G) T ->
  tsize_flat T < n ->
  stp G GH T T.
Proof.
  ...
  - (* TBind *)
    remember (open_r (varH (length GH)) T0) as TT.
    assert (stp G (TT :: GH) TT TT). eapply IHn. subst.
    eapply closed_open_r. simpl. eapply closed_upgrade_free. eassumption. omega.
    constructor. simpl. omega. subst. unfold open_r. erewrite <- open_r_preserves_size. simpl in H0. omega.
...
Lemma indexr_safe_ex: forall H1 GH G1 TF i,
             R_env H1 GH G1 ->
             indexr i G1 = Some TF ->
             exists d v, indexr i H1 = Some v /\ indexr i GH = Some d /\ (v,d) ⋵* (ty_vset GH [] TF).
Proof.
  intros. destruct H. destruct H2. auto.
Qed.
...
Lemma closed_no_open: forall T x h i j k,
  closed h i j k T ->
  T = open_rec i x T.
Proof.
...
Lemma closed_no_subst: forall T h i k TX,
   closed h i 0 k T ->
   subst TX T = T.
Proof.
  intros T. induction T; intros; inversion H; simpl; eauto;
  subst; try erewrite IHT1; try erewrite IHT2; try erewrite IHT; eauto. omega.
Qed.

Lemma closed_subst: forall h i j k V T, closed h i (j+1) k T -> closed 0 0 j k (TSel V) -> closed h i j k (subst V T).
Proof.
  intros. generalize dependent i. generalize dependent h.
  ...
  - (* Case "TVarH". *) simpl.
    ...
    eapply closed_upgrade_freer.
    eauto. omega. omega.
    econstructor.
    eapply beq_nat_false_iff in E. omega.
Qed.

Lemma closed_nosubst: forall h i j k V T, closed h i (j+1) k T -> nosubst T -> closed h i j k (subst V T).
Proof.
  intros. generalize dependent i. generalize dependent h.
  induction T; intros; inversion H;
  try econstructor;
...
(* TODO: adapt rest of file *)

Lemma subst_open_commute_m: forall h i j k k' j' V T2, closed 0 (i+1) (j+1) k T2 -> closed h 0 j' k' (TSel V) ->
    subst V (open_rec i (varH (j+1)) T2) = open_rec i (varH j) (subst V T2).
Proof.
  intros.
  generalize dependent i. generalize dependent j. generalize dependent h.

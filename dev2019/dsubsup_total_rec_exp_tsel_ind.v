...
| LTAll: forall Gamma GH n T1 T2 vsn1 vl1 vsn H T0 t,
    ...
    (exists (vs2 : vseta) vl2,
      tevaln (vl1 :: H) t vl2
       /\ forall k, logrel_ty Gamma (vs2 :: GH) (open (varH (length GH)) T2) k (vs2 k) vl2) ->
...
.
 ...
 (*         | _ => False *)
 (*         end *)
 (*       | (ttyp T) => val_type' env GH (inl T) n dd v *)
...
Program Fixpoint val_type' (env: list vseta) (GH:list vseta) (T_tm : ty + tm) n (dd: vset n) (v:vl) {measure (size_flat T_tm)}: Prop :=
  ...
  | inl T =>
    ...
    | vabs env1 T0 y, TAll T1 T2 =>
      ...
      forall jj vx,
        (forall kx, val_type' env GH (inl  T1) kx (jj kx) vx) ->
        exists (jj2:vseta) v, tevaln (vx::env1) y v /\ (forall k, val_type' env (jj::GH) (inl (open (varH (length GH)) T2)) k (jj2 k) v)
    ...
    | vty env1 TX, TMem T1 T2 =>
      ...
      match (vsmatch n dd) with
        ...
        | vsmatch (S n0) dd => forall (dy:vseta) vy,
            (val_type' env GH (inl T1) n0 (dy n0) vy -> dd (dy n0) vy) /\
            (dd (dy n0) vy -> val_type' env GH (inl T2) n0 (dy n0) vy)
      end

    | _, TSel tm => val_type' env GH (inr tm) n dd v

    | _, TAnd T1 T2 =>
      val_type'  env GH (inl T1) n dd v /\ val_type' env GH (inl T2) n dd v

    | _, TBind T1 =>
      closed 1 (length GH) (length env) T1 /\
      exists jj:vseta, jj n = dd /\ forall n, val_type' env (jj::GH) (inl (open (varH (length GH)) T1)) n (jj n) v
...
(* For compatibility with previous development *)
Definition val_type (env: list vseta) (GH:list vseta) (T : ty) n (dd: vset n) (v:vl) := val_type' env GH (inl T) n dd v.
...
Lemma closed_open: forall i j k V T, closed (i+1) j k T -> closed i j k (TSel (tvar V)) ->
...
Proof. induction T; intros; try destruct v; simpl in *.
  ...
  constructor.
  inversion H; subst.
  specialize (IHT1 _ _ _ _ _ _ H6 H0). assert (closed (S i) (j) k (TSel (tvar V))).
...
Proof. induction T; intros; try destruct v; simpl in *.
  ...
  constructor.
  inversion H0; subst.
  specialize (IHT1 _ _ _ _ _ _ H H6). assert (closed (S i) (j + 1) k (TSel (tvar V'))).

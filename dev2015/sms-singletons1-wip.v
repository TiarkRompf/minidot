...
Lemma stp2_subst_narrowZ: forall ml, forall nl, forall m m2 b GH G1 T2 TX x n1 n2,
   vtp m b G1 x (substt x TX) n1 ->
   htp m2 true (GH++[TX]) G1 0 T2 n2 -> x < length G1 -> m < ml -> n2 < nl ->
   vtpdd m b G1 x (substt x T2). (* must decrease b/c transitivity *)
Proof. 
  ...
  - Case "bind". subst.
    assert (vtpdd m b G1 x (substt x (TBind TX0))) as A.
    eapply IHnl. eauto. eauto. eauto. eauto. omega. eauto.
    destruct A as [? [? [A M]]]. inversion A. subst.
    ...
    unfold substt in R. rewrite R in H7. eapply H7. simpl. eauto. destruct M;[eauto|right;omega]. 
  - Case "sub". subst.
    eapply IHnl. eauto. eapply H4. eauto. eauto. omega.
Qed.


Lemma stp2_subst_narrow0: forall n, forall m2 b GH G1 T1 T2 TX x n2 n1 m,
...
Proof.
  ...
  assert (forall ni n2, forall m b GH T2 xi,
    htp m b (GH ++ [TX]) G1 xi T2 n2 -> xi <> 0 -> n2 < ni -> ni < S n ->
    htpd m b (map (substt x) GH) G1 (xi-1) (substt x T2)) as htp_subst_narrow02. {
      induction ni. intros. omega. 
      intros. inversion H3.
      + (* var *) subst.
        repeat eexists. eapply htp_var. eapply index_subst1. eauto. eauto.
        rewrite map_length. eauto. eapply closed_subst0. rewrite app_length in H8. eapply H8. eauto.
      + (* bind *) subst.
        assert (htpd m0 b0
         (map (substt x) (GH0)) G1
         (xi-1) (substt x (TBind TX0))) as BB.
        eapply IHni. eapply H7. eauto. omega. omega.
        rewrite subst_open5. 
      ...
      + (* sub *) subst.
        ...
        assert (stpd2 m b0 (map (substt x) GL0) G1 (substt x T3) (substt x T0)) as BB.
  ...
  (* special case *)                                                                                   
  assert (forall ni n2, forall m b T0 T2,
    htp m b (T0 :: GH ++ [TX]) G1 (length (GH ++ [TX])) T2 n2 -> n2 < ni -> ni < S n ->
    htpd m b (map (substt x) (T0::GH)) G1 (length GH) (substt x T2)) as htp_subst_narrow0. {
      intros.
  ...
  inversion H.
  - Case "bot". subst.
    eapply stpd2_bot; eauto. rewrite map_length. simpl. eapply closed_subst0. rewrite app_length in H3. simpl in H3. eapply H3. eauto.
  - Case "top". subst.
    eapply stpd2_top; eauto. rewrite map_length. simpl. eapply closed_subst0. rewrite app_length in H3. simpl in H3. eapply H3. eauto.
  - Case "bool". subst.
  ...
  - Case "varax". subst.
    ...
    + (* miss *)
      ...
      repeat eexists. unfold substt. simpl. rewrite E. eapply stp2_varax. rewrite map_length. rewrite app_length in H3. simpl in H3. omega. 
  - Case "vara1". 
    case_eq (beq_nat x0 0); intros E.
    + (* hit *)
      assert (x0 = 0). eapply beq_nat_true_iff. eauto. subst x0. 
      assert (vtpddd b G1 x (substt x T2)). subst. admit. (* eapply narrowX; eauto. omega. *)
      ev. eu. subst. repeat eexists. simpl. eapply stp2_var1. eauto. 
    + (* miss *)
      assert (x0 <> 0). eapply beq_nat_false_iff. eauto.
      subst. 
      assert (htpd m2 true (map (substt x) GH) G1 (x0-1) (substt x T2)). eapply htp_subst_narrow02. eauto. eauto. eauto. eauto.
...
*)
  ...
  - Case "sel1". subst. (* invert htp to vtp and create strong_sel node *)
    case_eq (beq_nat x0 0); intros E.
    + assert (x0 = 0). eapply beq_nat_true_iff. eauto. subst x0.
      assert (exists m0, vtpdd m0 b G1 x (substt x (TMem TBot T2))) as A. eexists. eapply stp2_subst_narrowZ. eauto. eauto. eauto. eauto. eauto.
      destruct A as [? [? A]].
      destruct A as [? [A ?]]. inversion A. subst. 
      (* assert (ms = m2) as M. admit. subst. *)
      repeat eexists. eapply stp2_strong_sel1. eauto. unfold substt. eauto.
      repeat eexists. eapply stp2_strong_sel1. eauto. unfold substt. eauto.
    + assert (x0 <> 0). eapply beq_nat_false_iff. eauto.
  ...
  - Case "sel2". subst. (* invert htp to vtp and create strong_sel node *)
    case_eq (beq_nat x0 0); intros E.
    + assert (x0 = 0). eapply beq_nat_true_iff. eauto. subst x0.
      assert (exists m0, vtpd m0 true G1 x (substt x (TMem T1 TTop))) as A. eapply narrowX. eauto. omega.
      destruct A as [? A]. eu. inversion A. subst. 
      assert (ms = m2) as M. admit. subst. 
      repeat eexists. eapply stp2_strong_sel2. eauto. unfold substt. eauto.
    + assert (x0 <> 0). eapply beq_nat_false_iff. eauto.
      eapply htp_subst_narrow02 in H2. 
      eu. repeat eexists. unfold substt. simpl. rewrite E. eapply stp2_sel2. eapply H2. eauto. eauto. eauto. 

   - Case "bind1". 
    assert (htpd m2 true (map (substt x) (T1'::GH)) G1 (length GH) (substt x T2)). 
  ...
  - Case "bindx". 
    assert (stpd2 m true (map (substt x) (T1'::GH)) G1 (substt x T1') (substt x T2')).
    eapply IHn. eauto. eauto. omega. eauto.
    eu. repeat eexists. eapply stp2_bindx. eapply H14.
...
*)
  ...
  - Case "bind2". 
    assert (htpd m2 true (map (substt x) (T1::GH)) G1 (length GH) (substt x T2')). 
    eapply htp_subst_narrow0. eauto. eauto. omega. 
    eu. repeat eexists. eapply stp2_bind2. rewrite map_length. eapply H13.
    simpl. subst T2'. fold subst. eapply subst_open4.
    fold subst. eapply closed_subst0. rewrite app_length in H4. simpl in H4. rewrite map_length. eauto. eauto. 
    eapply closed_subst0. rewrite map_length. rewrite app_length in H5. simpl in H5. eauto. eauto.
  ...
  - Case "transf". 
    assert (stpd2 m b (map (substt x) GH) G1 (substt x T1) (substt x T3)).
    eapply IHn; eauto. omega.
    assert (stpd2 m0 b (map (substt x) GH) G1 (substt x T3) (substt x T2)).
    eapply IHn; eauto. omega.
    eu. eu. repeat eexists. eapply stp2_transf. eauto. eauto. eauto. eauto.  
...
Lemma stp2_subst_narrowX: forall ml, forall nl, forall m m2 GH G1 T2 TX x n1 n2,
   vtp m true G1 x (substt x TX) n1 ->
   htp m2 true (GH++[TX]) G1 0 T2 n2 -> x < length G1 -> m < ml -> n2 < nl ->
   (forall (m0 m1 : nat) (G1 : venv) x (T2 T3 : ty) (n1 n2 : nat),
        vtp m0 true G1 x T2 n1 ->
        stp2 m1 true [] G1 T2 T3 n2 -> m0 <= m ->
        vtpdd m0 true G1 x T3) ->
   vtpdd m true G1 x (substt x T2). (* must decrease b/c transitivity *)
Proof. 
  intros ml. (* induction ml. intros. omega. *)
  intros nl. induction nl. intros. omega.
  intros.
  ...
  - Case "var". subst.
    ...
    repeat eexists. eauto. eauto. 
  - Case "bind". subst.
    assert (vtpdd m true G1 x (substt x (TBind TX0))) as A.
    eapply IHnl. eauto. eauto. eauto. eauto. omega. eauto.
    destruct A as [? [? [A M]]]. inversion A. subst.
    repeat eexists. unfold substt. erewrite subst_open_commute0.
    assert (closed 0 (length G1) 0 (TBind (substt x TX0))). eapply vtp_closed. unfold substt in A. simpl in A. eapply A.
    assert ((substt x (TX0)) = TX0) as R. eapply subst_closed_id. eauto.
    unfold substt in R. rewrite R in H8. eapply H8. simpl. eauto. destruct M;[eauto|right;omega]. 
  - Case "sub". subst.
    admit.
    (*
    assert (GL = [TX]). eapply sub_env1; eauto. subst GL.
    assert (vtpdd m true G1 x (substt x T1)) as A.
    eapply IHnl. eauto. eauto. eauto. eauto. omega. eauto. 
    eu.
    assert (stpd2 m2 true (map (substt x) []) G1 (substt x T1) (substt x T2)) as B.
    eapply stp2_subst_narrow0. eauto. eauto. eauto. {
      intros. eapply IHnl in H. destruct H as [? [? [? ?]]]. repeat eexists. eauto. eauto. eauto. eauto. omega. eauto. 
    }
    simpl in B. eu. 
    assert (vtpdd x0 true G1 x (substt x T2)).
...
Lemma stp2_trans: forall l, forall n, forall k, forall m1 m2 G1 x T2 T3 n1 n2,
  vtp m1 true G1 x T2 n1 -> 
  stp2 m2 true [] G1 T2 T3 n2 ->
  m2 < l -> n2 < n -> n1 < k -> 
  vtpddd true G1 x T3.
Proof.
...
(*    + SCase "ssel2".
      assert (vtpdd m1 true G1 x TX). eapply IHn. eauto. eauto. eauto. omega. eauto. 
      eu. repeat eexists. eapply vtp_sel. eauto. eauto. eauto.
    + SCase "sel2".
      eapply stp2_closed2 in H0. simpl in H0. inversion H0. inversion H9. omega. *)
    + SCase "trans".
      assert (vtpddd true G1 x T0) as LHS. eapply IHn. eauto. eauto. omega. omega. eauto.  eu.
      assert (vtpddd true G1 x T3) as BB. eapply IHn. eapply LHS. eauto. omega. omega. eauto. eu.
      repeat eexists. eauto. 
  - Case "bool". inversion H0; subst; invty.
    + SCase "top". repeat eexists. eapply vtp_top. eapply index_max. eauto. 
    + SCase "bool". repeat eexists; eauto. 
    (*+ SCase "ssel2". 
      assert (vtpdd m1 true G1 x TX). eapply IHn; eauto. omega. 
      eu. repeat eexists. eapply vtp_sel. eauto. eauto. eauto.
    + SCase "sel2". 
      eapply stp2_closed2 in H0. simpl in H0. inversion H0. inversion H9. omega. *)
    + SCase "trans".
  ...
  - Case "mem". inversion H0; subst; invty.
    ...
    + SCase "mem". invty. subst.
      ...
      eapply stp2_transf. eauto. eapply H5. instantiate (1:= ms+m2). omega. omega. 
      eapply stp2_transf. eauto. eapply H14. omega. omega.
    (* + SCase "sel2". 
      assert (vtpdd m1 true G1 x TX0). eapply IHn; eauto. omega. 
      eu. repeat eexists. eapply vtp_sel. eauto. eauto. eauto. 
    + SCase "sel2". 
      eapply stp2_closed2 in H0. simpl in H0. inversion H0. inversion H11. omega. *)
    + SCase "trans".
      assert (vtpddd true G1 x T5) as LHS. eapply IHn. eauto. eauto. omega. omega. eauto. eu.
      assert (vtpddd true G1 x T3) as BB. eapply IHn. eapply LHS. eauto. omega. omega. eauto. eu.
      repeat eexists. eauto. 
  - Case "fun". inversion H0; subst; invty.
    ...
    + SCase "fun". invty. subst.
      ...
      eapply stp2_transf. eauto. eapply H6. instantiate (1:= ms+m2). omega. omega. 
      eapply stp2_transf. eauto. eapply H15. omega. omega. 
    (* + SCase "sel2". 
      assert (vtpdd m1 true G1 x TX). eapply IHn; eauto. omega. 
      eu. repeat eexists. eapply vtp_sel. eauto. eauto. eauto. 
    + SCase "sel2". 
    ...
    + SCase "trans".
      assert (vtpddd true G1 x T7) as LHS. eapply IHn. eauto. eauto. omega. omega. eauto. eu.
      assert (vtpddd true G1 x T3) as BB. eapply IHn. eapply LHS. eauto. omega. omega. eauto. eu.
      repeat eexists. eauto. 
  - Case "bind1". 
    inversion H0; subst; invty.
    + SCase "top". repeat eexists. eapply vtp_top. eapply vtp_closed1. eauto. 
    (* + SCase "sel2". 
      assert (vtpdd (S m) true G1 x TX). eapply IHn; eauto. omega. 
      eu. repeat eexists. eapply vtp_sel. eauto. eauto. eauto. 
    + SCase "sel2". 
    ...
    + SCase "bind1".
      ...
      (* left *)
      assert (vtpd m true G1 x (open 0 VX T0)) as LHS. eexists. eassumption.
      eu.
      (* right *)
      ...
      assert (vtpdd m true G1 x (substt x T3)) as BB. {
        eapply stp2_subst_narrowX. rewrite <-R in LHS. eapply LHS.
        instantiate (2:=nil). simpl. eapply H12. eapply vtp_closed1. eauto. eauto. eauto. 
        { admit. (* intros. eapply IHl. eauto. eauto. omega. eauto. eauto.*) }
      }
      rewrite R1 in BB. 
    ...
    + SCase "bindx".
      ...
      assert (stpd2 m0 true (map (substt x) []) G1 (substt x (open 0 VZ T0)) (substt x (open 0 VZ T4))) as B. {
        eapply stp2_subst_narrow0. rewrite app_nil_l. eapply H12. eapply vtp_closed1. eauto. eauto.
        { admit. }
      }
      eu. simpl in B. 
      (* STP is bigger than our input, so we need to use induction on LHS *)
      assert (vtpddd true G1 x (substt x (open 0 VZ T4))) as BB. {
        eapply IHl. rewrite <-R in LHS. eauto. eauto. omega. eauto. eauto. 
      }
      unfold substt in BB. subst. erewrite subst_open_commute0 in BB. 
      clear R.
      eu. repeat eexists. eapply vtp_bind. eauto. eauto. eauto. (* enough slack to add bind back *)
    + SCase "trans".
      assert (vtpddd true G1 x T4) as LHS. eapply IHn. eauto. eauto. omega. omega. eauto. eu.
      assert (vtpddd true G1 x T3) as BB. eapply IHn. eapply LHS. eauto. omega. omega. eauto. eu.
      repeat eexists. eauto. 
  - Case "ssel2". subst. inversion H0; subst; invty.
    + SCase "top". repeat eexists. eapply vtp_top. eapply vtp_closed1. eauto. 
    + SCase "ssel1". index_subst. eapply IHn. eauto. eauto. eauto. omega. eauto.
      (* NOTE: here we require that upper comparison from strong_sel1 is true *)
    ...
    + SCase "trans".
      assert (vtpddd true G1 x T2) as LHS. eapply IHn. eauto. eauto. omega. omega. eauto. eu.
      assert (vtpddd true G1 x T3) as BB. eapply IHn. eapply LHS. eauto. omega. omega. eauto. eu.
      repeat eexists. eauto. 
...
WHY?
    ...
    vtp (TMem S T) --->  vtp (TMem S {z =>T})

inverting this would give us back T < {z=>T}
in strong_sel1
...
(*
Lemma stp2_subst_narrowY: forall ml, forall nl, forall m m2 GH G1 T2 TX x n1 n2,
   vtp m true G1 x (substt x TX) n1 ->
   htp m2 false (GH++[TX]) G1 0 T2 n2 -> x < length G1 -> m < ml -> n2 < nl ->
   (forall (m0 m1 : nat) (G1 : venv) x (T2 T3 : ty) (n1 n2 : nat),
        vtp m0 true G1 x T2 n1 ->
        stp2 m1 true [] G1 T2 T3 n2 -> m0 <= m ->
        vtpddd true G1 x T3) ->
  vtpddd true G1 x (substt x T2). (* decrease b/c transitivity *)
Proof. 
  intros ml. (* induction ml. intros. omega. *)
  intros nl. induction nl. intros. omega.
  intros.
  ...
  - Case "var". subst.
    assert (T2 = TX). eapply index_hit0. eauto. 
    subst T2.
    admit. (* repeat eexists. eauto. eauto. *)
  - Case "bind". subst.
    assert (vtpddd true G1 x (substt x (TBind TX0))) as A.
    eapply IHnl. eauto. eauto. eauto. eauto. omega. eauto.
    destruct A as [? [? A]]. inversion A. subst.
    repeat eexists. unfold substt. erewrite subst_open_commute0.
    assert (closed 0 (length G1) 0 (TBind (substt x TX0))). eapply vtp_closed. unfold substt in A. simpl in A. eapply A.
    assert ((substt x (TX0)) = TX0) as R. eapply subst_closed_id. eauto.
    unfold substt in R. rewrite R in H8. eapply H8. simpl. eauto.
  - Case "sub". subst. 
    assert (GL = [TX]). eapply sub_env1; eauto. subst GL.
    assert (vtpddd true G1 x (substt x T1)) as A.
    eapply IHnl. eauto. eauto. eauto. eauto. omega. eauto. 
    eu.
    assert (stpd2 m2 false (map (substt x) []) G1 (substt x T1) (substt x T2)) as B.
    eapply stp2_subst_narrow0. eauto. eauto. eauto. {
      intros. eapply stp2_subst_narrowX in H. destruct H as [? [? [? ?]]]. repeat eexists. eauto. eauto. eauto. eauto. eauto. {
        intros. eapply stp2_trans. eauto. eauto. eauto. eauto. eauto.
    }}
    simpl in B. eu. 
    eapply 
    
    assert (vtpdd x0 true G1 x (substt x T2)). admit. (* conv false to true ! *)
    eapply H4. eauto. eauto. destruct M as [M|M]. inversion M. eauto. 
    destruct H9 as [? [? [H9 M1]]]. repeat eexists. eauto. destruct M as [M|M]. inversion M. destruct M1;[eauto|right;omega]. 
Qed.
*)
...
Lemma stp2_trans2: forall n, forall m0 m1 G1 T2 T3 x n1 n2,
  vtp m0 true G1 x T2 n1 ->
  stp2 m1 false [] G1 T2 T3 n2 -> n2 < n ->
  vtpddd true G1 x T3.
Proof.
  intros n. induction n. intros. omega.
  intros.
...
- Case "top". inversion H0; subst; invty.
    + SCase "ssel2".
      assert (vtpddd true G1 x TX). eapply IHn. eauto. eauto. omega. 
      ev. eu. repeat eexists. eapply vtp_sel. eauto. eauto. 
    + SCase "sel2".
      eapply stp2_closed2 in H0. simpl in H0. inversion H0. inversion H7. omega.
    + SCase "bind2".
      remember (TVar false (length [])) as VZ.
      remember (TVar true x) as VX.
      (* left *)
      assert (vtpd m0 true G1 x (open 0 VX TTop)) as LHS. eexists. eassumption.
      eu.
      (* right *)
      assert (substt x (open 0 VZ TTop) = (open 0 VX TTop)) as R. unfold substt. subst. eapply subst_open_commute0. econstructor.

      assert (exists m,vtpdd m true G1 x (substt x (open 0 VZ T0))) as BB. {
        eexists. eapply stp2_subst_narrowX. rewrite <-R in LHS. eapply LHS.
        instantiate (2:=nil). simpl. eapply H9. eauto. eauto. eauto.
        { intros. eapply stp2_trans; eauto. }
      }
      unfold substt in BB. subst. erewrite subst_open_commute0 in BB.                                       ev. eu. repeat eexists. eapply vtp_bind. eauto. eauto. eauto.
    + SCase "wrap".
      eapply stp2_trans in H9. eu. repeat eexists. eauto. eauto. eauto. eauto. eauto. 
    + SCase "trans". 
      assert (vtpddd true G1 x T0) as LHS. eapply IHn. eauto. eauto. eauto. omega. eu.
      assert (vtpddd true G1 x T3) as BB. eapply IHn. eapply LHS. eauto. omega.
      eauto.
  - Case "bool". inversion H0; subst; invty.
    + SCase "ssel2". 
      assert (vtpdd m1 true G1 x TX). eapply IHn; eauto. omega. 
    ...
    + SCase "sel2". 
      eapply stp2_closed2 in H0. simpl in H0. inversion H0. inversion H9. omega.
    + SCase "trans".
      assert (vtpdd m1 true G1 x T0) as LHS. eapply IHn. eauto. eauto. eauto. omega. eauto. eu.
      assert (vtpdd x0 true G1 x T3) as BB. eapply IHn. eapply LHS. eauto. omega. omega. eauto. eu.
  ...
  - Case "mem". inversion H0; subst; invty.
    ...
    + SCase "sel2". 
      eapply stp2_closed2 in H0. simpl in H0. inversion H0. inversion H11. omega.
    + SCase "trans".
      assert (vtpdd m1 true G1 x T5) as LHS. eapply IHn. eauto. eauto. eauto. omega. eauto. eu.
      assert (vtpdd x0 true G1 x T3) as BB. eapply IHn. eapply LHS. eauto. omega. omega. eauto. eu.
      repeat eexists. eauto. omega. 
  - Case "fun". inversion H0; subst; invty.
    + SCase "top". repeat eexists. eapply vtp_top. eapply index_max. eauto. eauto. 
    + SCase "fun". invty. subst.
      ...
      eapply stp2_transf. eauto. eapply H15.
      eauto. 
    + SCase "sel2". 
      assert (vtpdd m1 true G1 x TX). eapply IHn; eauto. omega. 
      eu. repeat eexists. eapply vtp_sel. eauto. eauto. eauto. 
    + SCase "sel2". 
      eapply stp2_closed2 in H0. simpl in H0. inversion H0. inversion H12. omega.
    + SCase "trans".
      assert (vtpdd m1 true G1 x T7) as LHS. eapply IHn. eauto. eauto. eauto. omega. eauto. eu.
      assert (vtpdd x0 true G1 x T3) as BB. eapply IHn. eapply LHS. eauto. omega. omega. eauto. eu.
      repeat eexists. eauto. omega. 
  ...
  - Case "down". subst.
    ...
    + inversion H0.
      * SSCase "mem". admit.
      * SSCase "bind". subst. admit. (* XXX TODO XXX *)
      * SSCase "wrapf". admit.
      * SSCase "transf". admit.

(* WHAT IS OUR RESTRICTION?? 

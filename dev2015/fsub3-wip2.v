...
| stp2_strong_sel1: forall G1 G2 GX TX x T2 GH STO n1,
...
| stp2_strong_sel2: forall G1 G2 GX TX x T1 GH STO n1,
...
| stp2_sel1: forall G1 G2 TX x T2 GH STO n1 n2 v,
    index x G1 = Some v ->
...
| stp2_sel2: forall G1 G2 TX x T1 GH STO n1 n2 v,
    index x G2 = Some v ->
    val_type STO (base v) v TX ->
    closed 0 0 TX ->
...
Proof.
  ...
  - Case "cell". subst. inversion H1; subst.
    + SCase "top".
      assert (stpd2 false G1 T0 G1 T0 []) as A0 by solve [eapply stpd2_wrapf; eapply stp2_reg1; eassumption].
      inversion A0 as [na0 HA0].
      eexists. eapply stp2_top. eapply stp2_cell. eassumption. eassumption.
    + SCase "cell".
      assert (stpd2 false G3 T2 G1 T0 []) as A by solve [eapply stpd2_trans; eauto].
      inversion A as [na A'].
      assert (stpd2 false G1 T0 G3 T2 []) as B by solve [eapply stpd2_trans; eauto].
      inversion B as [nb B'].
...
Lemma stp2_substitute_aux: forall n, forall m G1 G2 T1 T2 GH n1,
   ...
   n1 <= n ->
   forall STO GH0 GH0' GX TX T1' T2' V,
     GX = base V ->
     GH = (GH0 ++ [(0,(GX, TX))]) ->
     val_type STO (base V) V TX ->
     closed 0 0 TX ->
...
Proof.
  ...
  - Case "topx".
    intros STO GH0 GH0' GXX TXX T1' T2' V ? ? ? CX IX1 IX2 FA.
  ...
  - Case "sel1".
    intros STOX GH0 GH0' GXX TXX T1' T2' V ? ? ? CX IX1 IX2 FA.

    assert (length GH = length GH0 + 1). subst GH. eapply app_length.
    assert (length GH0 = length GH0') as EL. eapply Forall2_length. eauto.

    eapply (compat_sel STO GXX TXX (Some V) G1 T1' (base v) TX) in IX1. repeat destruct IX1 as [? IX1].
  ...
  - Case "sel2".
    ...
    eapply (compat_sel STO GXX TXX (Some V) G2 T2' (base v) TX) in IX2. repeat destruct IX2 as [? IX2].
  ...
  - Case "selx".
    ...
    assert (T1' = TSel x1). {
      destruct IX1. ev. eauto. destruct H5. ev. auto. ev. eauto.
    }
    assert (T2' = TSel x2). {
      destruct IX2. ev. eauto. destruct H6. ev. auto. ev. eauto.
    }
    subst.
  ...
  - Case "all".
    intros STO GH0 GH0' GX TX T1' T2' V ? ? ? CX IX1 IX2 FA.
...
Proof.
  ...
  - Case "App".
    ...
    +
      ...
      assert (exists senv', res_type venv0 (senv'++senv) rx T1) as HRX. SCase "HRX". subst. eapply IHn; eauto.
      ...
      assert (exists senv', res_type venv0 (senv'++(senvx'++senv)) rf (TFun T1 T2)) as HRF. SCase "HRF". subst. eapply IHn; eauto. apply wf_env_sto_ext. assumption.
      ...
      assert (exists senv', res_type ((x0,vx)::(f0,vf)::env1) (senv'++senvf'++senvx'++senv) res T4) as HRY.
        SCase "HRY".
          subst. eapply IHn. eauto. eauto.
          (* wf_env f x *) econstructor. eapply valtp_sto_ext. eapply valtp_widen; eauto. eapply sstpd2_extend2. eapply sstpd2_extend2. eauto. eauto. eauto.
    ...
    + assert (
          exists senv',
            res_type venv0 (senv' ++ senv) res T1) as A. {
        eapply IHhas_type; eauto.
      }
      destruct A as [senv' A].
      exists senv'. eapply restp_widen. eapply A. eapply stpd2_upgrade. eapply stp_to_stp2; eauto. econstructor.

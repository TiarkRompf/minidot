...
Proof.
  ...
  eapply stp_bindx. eauto. crush2. crush2.
  unfold open. simpl.
  eapply stp_all. eapply stp_bindx. eauto. crush2. crush2.
  unfold open. simpl.
  eapply stp_and2. eapply stp_and11; crush2.  eapply stp_and12; crush2.
  unfold open. simpl.
  eapply stp_and2. eapply stp_and11; crush2.  eapply stp_and12; crush2.
...
Proof.
  ...
  - Case "selb1".
    ...
    assert (closed 0 (length ([]:aenv)) (TBind (TMem l TBot T0))). {
    ...
    right. left. split. eapply closed_open; eauto.  reflexivity.
    right. left. split. eapply closed_open; eauto.  reflexivity.
  ...
  - Case "selb2".
    ...
    assert (closed 0 (length ([]:aenv)) (TBind (TMem l T0 TTop))). eapply stp2_closed2; eauto.
    simpl in  H7. unfold closed in H9. inversion H9. subst. inversion H13. subst.
  ...
  - Case "selab1".
    ...
    + SCase "x = 0". ev. subst.
      ...
      * subst. simpl. inversion H11. subst.
        ...
        assert (exists nb, stp2 (S d) false GXX (subst TXX' TXX) G2 (TBind (TMem l TBot T0)) [] nb) as SBind. {
     ...
     (subst (varF x2) (open (varH 0) T2))) as CPH. {
        ...
        assert (T2' = open (varF x) T0) as C2. {
  ...
  - Case "selab2".
    ...
    + SCase "x = 0". ev. subst.
      ...
      * subst. simpl. inversion H11. subst.
        ...
        assert (exists nb, stp2 (S d) false GXX (subst TXX' TXX) G1 (TBind (TMem l T0 TTop)) [] nb) as SBind. {
          eapply IHn. eauto. omega. rewrite app_nil_l. eauto. eauto. eapply CX. eauto. eauto.
          right. left. split. eassumption. reflexivity.
          eauto. eauto.
        }
     ...
     (subst (varF x2) (open (varH 0) T2))) as CPH. {
        ...
        assert (T1' = open (varF x) T0) as C2. {
          unfold open. erewrite open_noop. eassumption. eassumption.
        }
        rewrite C2.
        eapply IHn in H9. destruct H9.
...
Proof.
  ...
  assert (exists y T3 T4, index l ds = Some (dfun (1 + (fresh venv1))  y) /\ has_type ((1+(fresh venv1), (open (varF (fresh venv1)) T3)) :: ((fresh venv1), (open (varF (fresh venv1)) T)) :: tenv0) y (open (varF (fresh venv1)) T4) /\ sstpd2 true (((fresh venv1), vobj venv1 (fresh venv1) ds)::venv1) (open (varF (fresh venv1)) (TFun l T3 T4)) venv0 (TFun l T1 T2) []) as A. {
    ...
    unfold id in H4.
    remember ((fresh venv1, vobj venv1 (fresh venv1) ds) :: venv1) as venv.
    assert (sstpd2 true venv (open (varF (fresh venv1)) T) venv0 (TFun l T1 T2) []) as B. { eexists; eassumption. }
    ...
    split. rewrite <- A. rewrite H0. eassumption.
    destruct (tand_shape (TFun m T0 T3) TS).
    rewrite H4 in H3. rewrite H3 in B. destruct B as [? B]. unfold open in B. simpl in B. inversion B. eapply beq_nat_true in E2. rewrite <- E2 in H8. eexists. eassumption.
    eapply dcs_has_type_stp in H2. inversion H2. rewrite <- H1. eapply beq_nat_true in E2. rewrite <- E2. eexists. eassumption.
    ...
    rewrite H4 in H3. rewrite H3 in B. destruct B as [? B]. inversion B. unfold open in H8. simpl in H8. inversion H8. apply beq_nat_false in E2. omega. eexists. eassumption.
    ...
    eapply dcs_has_type_stp in H2. inversion H2. rewrite <- H. eapply beq_nat_true in E2. rewrite <- E2. eexists. eassumption.
...
Proof.
  intros. induction H; eauto.
  - inversion IHhas_type1. subst. assumption. rewrite H1 in H2. inversion H2.
...
*)
  - Case "App".
    ...
    +
      ...
      assert (res_type venv0 rf (TFun i T1 T2)) as HRF. SCase "HRF". subst. eapply IHn; eauto.
      inversion HRF as [? vf].

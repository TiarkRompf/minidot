    ...
    | vty G T  => 
      (fix tsize H T := 
         match T with
           ...
           | TSel (varB x) => S (* can't open in TAll, hence implement reverse lookup *)
             ((fix idx x H := 
                match H with
...
Fixpoint tsize (G:venv) (H:list nat) (T:ty) {struct T} :=
  match T with
    ...
    | TSel (varB x)  => S (* reverse lookup -- not easy to use SelH in TAll *)
...
Proof.
  ...
  - Case "sel". simpl. destruct v; eauto. 
    ...
    + induction (rev HX).
      * simpl. eauto.
      * simpl. destruct (beq_nat i (length l)). eauto. apply IHl.
...
Inductive bounds : venv -> aenv -> ty -> (venv*ty) -> (venv*ty)  -> Prop :=
...
| bs_selb: forall G GH GX TX GT1 GT2 x,
    indexr x GH = Some (GX, TX) ->
    bounds GX GH TX GT1 GT2 ->
    bounds G GH (TSel (varH x)) GT1 GT2
...
Definition ok_env_size GH GHN := (forall GX TX x,
     indexr x GH = Some (GX,TX) ->
     exists n, indexr x GHN = Some n /\ n = tsize GX GHN TX).
...
Proof.
  ...
  - (* sel *) simpl in H. rewrite H2 in H. rewrite <-tsz_eq in H.
              assert (tsize GU GHN TU < n). eapply (IHn GX GH GHN TX). omega. eauto.
              eauto. omega.
  - (* selb *) simpl in H.
              eapply H1 in H2. destruct H2 as [? [? ?]]. rewrite H2 in H.  
              assert (tsize GU GHN TU < n). eapply (IHn GX GH GHN TX). omega. eauto.
              eauto. omega.
Qed.
...
Proof.
  intros. eapply bounds_tsize_aux. eauto. eauto. eauto.
Qed.


Require Coq.Program.Wf.

Definition SET := (vl -> Prop).

(* Definition sel_type p G GH f v T := forall GL TL GU TU, bounds env f *)

(* PROBLEMS:

- bounds need reference to GHT environment (i.e if T1 = x.T we need to lookup x:(L..U))

- upper bound TU needs to be smaller than T1, so sizes must be relative to env, too

- cannot narrow in the classical sense, because of function contravariance
...
- problem case y.T -> T2
  ...
  y:(L..U),  val_type vy U   and U not smaller than y.T because from env

  but in this case we can use the corresponding jj from env!!

CAVEAT: not clear how this works with inv_bounds and stp2!!
...
Definition vtype (T0:ty) := (forall (env: venv), list (vl->Prop) -> aenv -> forall GHN: list nat, vl -> forall T: ty, (tsize env GHN T < tsize env GHN T0) -> Prop).


Definition foobar (T0:ty) (f: vtype T0) := True.


Inductive boundsx : forall T, vtype T -> venv -> list (vl->Prop) -> aenv -> list nat -> vl -> ty -> Prop :=
| bs_memx: forall G GH GHT GHN v T1 T2 TMAX VT,
    forall A: tsize G GHN T2 < tsize G GHN TMAX, 
    (VT:vtype TMAX) G GH GHT GHN v T2 A ->
    boundsx TMAX VT G GH GHT GHN v (TMem T1 T2).
...
Program Fixpoint val_type (env:venv) (GH:list ((vl->Prop))) GHT GHN (v:vl) (T:ty) {measure (tsize env GHN T)}: Prop :=
  match v,T with
    | vabs env1 T0 y, TAll T1 T2 =>
      closed 0 (length GH) (length env) T1 /\ closed 1 (length GH) (length env) T2 /\
      forall vx (jj:vl->Prop),
        val_type env GH GHT GHN vx T1 ->
        length GH = length GHN ->
        length GHT = length GHN ->
        (forall vy, jj vy -> boundsx env GH GHT GHN T1) ->
        exists v, tevaln (vx::env1) y v /\ val_type env (jj::GH) ((env,T1)::GHT) (tsize env GHN T1::GHN) v (open (varH (length GH)) T2)
    | vty env1 TX, TMem T1 T2 =>
      closed 0 0 (length env1) TX /\ (* required to convert to val_type_stub *)
      (* exists n1 n2,
        stp2 false false env1 TX env T2 GHT n1 /\ (* FIXME: what about [] vs GH ?? *)
        stp2 false false env T1 env1 TX GHT n2 *)
      (forall vx, val_type env GH GHT GHN vx T1 -> val_type env GH GHT GHN vx T2)
    | _, TSel (varF x) =>
      match indexr x env with
        | Some (vty GX TX) => val_type GX GH GHT GHN v TX
        | _ => False
      end
    | _, TSel (varH x) =>
...
Next Obligation.
  clear H1.
  simpl. omega. Qed. (* TApp case: vx / bounds *)
Next Obligation.
  simpl. unfold open. rewrite <-open_preserves_tsize.
  omega. simpl. omega. simpl. omega. Qed. (* TApp case: open *)
Next Obligation. simpl. omega. Qed. (* TMem case *)
Next Obligation. simpl. omega. Qed. (* TMem case *)
...
Lemma val_type_unfold: forall env GH GHT GHN v T, val_type env GH GHT GHN v T =
   match v,T with
    | vabs env1 T0 y, TAll T1 T2 =>
      closed 0 (length GH) (length env) T1 /\ closed 1 (length GH) (length env) T2 /\
      forall vx (jj:vl->Prop),
        ...
        length GH = length GHN ->
        length GHT = length GHN ->
        (forall vy, jj vy -> forall GL TL GU TU, bounds env GHT (* FIXME *) T1 (GL,TL) (GU,TU) -> tsize GU GHN TU < S (tsize env GHN T1) -> val_type GU GH GHT GHN vy TU) ->
        exists v, tevaln (vx::env1) y v /\ val_type env (jj::GH) ((env,T1)::GHT) (tsize env GHN T1::GHN) v (open (varH (length GH)) T2)
    | vty env1 TX, TMem T1 T2 =>
      closed 0 0 (length env1) TX /\ (* required to convert to val_type_stub *)
      (forall vx, val_type env GH GHT GHN vx T1 -> val_type env GH GHT GHN vx T2)
...
Proof.
  ...
  unfold_sub val_type (val_type env GH GHT GHN v T).
  simpl.
...
Lemma valtp_closed: forall vf GH GHT GHN H1 T1,
  val_type H1 GH GHT GHN vf T1 ->
...
Inductive vtp: venv -> (list (vl->Prop)) -> aenv -> (list nat) -> vl -> ty -> Prop :=
| vv: forall G H GHT GHN v T, val_type G H GHT GHN v T -> vtp G H GHT GHN v T.


Lemma unvv: forall G H GHT GHN v T,
  vtp G H GHT GHN v T -> val_type G H GHT GHN v T.
Proof.
...
Lemma inv_bounds: forall n m b H1 H2 T0 T4 GH GHT GHN vx GL1 TL1 GU1 TU1 n0 GH1 GH2,
    val_type H2 GH GHT GHN vx T4 -> 
    stp2 m b H2 T4 H1 T0 GH1 n0 -> n0 < n ->
    bounds H1 GH2 T0 (GL1, TL1) (GU1, TU1) ->
    exists GL TL GU TU n, bounds H2 GH2 T4 (GL, TL) (GU, TU) /\
                        stp2 false false GU TU GU1 TU1 GH1 n /\ n <= n0.
    Proof.
      admit. (*intros n. induction n. intros. omega.
      intros. 
      inversion H4; subst.
      - (* mem *)
        inversion H0; subst.
...
Lemma valtp_extendH: forall vf H1 GH GHT GHN T1 jj,
  val_type H1 GH GHT GHN vf T1 -> 
  vtp H1 (jj::GH) GHT GHN vf T1.
Proof.
  admit.
  (* NOTE: may be difficult due to contravariance for function arg.
     do we need the reverse direction as well? *)
Qed.
  
Lemma valtp_widen_aux: forall n, forall n1 m b vf GHT GHN H1 H2 GH GH1 T1 T2,
  val_type H1 GH GHT GHN vf T1 -> 
  ...
  (exists min, n1 <= min /\
               forall x HX TX, indexr x GH1 = Some (HX,TX) ->
                   exists v jj,
                     indexr x GH = Some (jj) /\
                     val_type HX GH GHT GHN v TX /\
                     (forall vy, jj vy ->
                                 forall m b G2 T2 ns,
                                   stp2 m b HX TX G2 (TMem TBot T2) GH1 ns -> ns < min ->
                                   vtp G2 GH GHT GHN vy T2
                     )) ->
  vtp H2 GH GHT GHN vf T2.
Proof.
  intros n. induction n. intros. omega.
  intros. 
  inversion H0.
  ...
  - Case "Fun".
    subst.
    rewrite val_type_unfold in H.
    assert (val_type H2 GH GHT GHN vf (TAll T4 T5)). rewrite val_type_unfold.
    ...
    intros vx jj VST0 LLSN LLTN STJ. 
    
    
    assert (val_type H2 GH GHT GHN vx T4) as VX0. { eapply VST0. }
    assert (vtp H1 GH GHT GHN vx T0) as VX1. {
    ...
    assert (forall (vy : vl),
             ...
             forall (GL : venv) (TL : ty) (GU : venv) (TU : ty),
               bounds H1 GHT T0 (GL, TL) (GU, TU) ->
               tsize GU GHN TU < S (tsize H1 GHN T0) ->
               val_type GU GH GHT GHN vy TU) as STJ1.
    { intros vy jjvy ? ? ? ? BB TS. specialize (STJ vy jjvy).
      
      (* downgrade bounds from H1 T0 to H2 T4, get size < n0 *)
      assert (exists GL2 TL2 GU2 TU2 n,
                bounds H2 GHT T4 (GL2, TL2) (GU2, TU2) /\
                stp2 false false GU2 TU2 GU TU GH1 n /\ n <= n0). {
        eapply inv_bounds. eapply VX0. eapply H6. info_eauto. eapply BB. }
      ev.
      assert (tsize x1 GHN x2 < S (tsize H2 GHN T4)). {
        eapply bounds_tsize. eapply H13. admit. (* ok_env GHT GHN *)
      }
      eapply unvv. eapply IHn. eapply STJ. eapply H13. eapply H17. eapply H14. omega. eapply H4.
      ev. exists x4. split. omega. eapply H16. 
    }

    eapply unvv in VX1. 
    destruct (LR vx jj VX1 LLSN LLTN STJ1) as [v [TE VT]]. 
    ...
    (* change type as well *)
    Definition has_bounds GX (GH: list (vl->Prop)) GHT GHN TX := forall i jj vy,
        indexr i GH = Some jj ->                                        
        jj vy ->
        forall (GL : venv) (TL : ty) (GU : venv) (TU : ty),
        bounds GX GHT TX (GL, TL) (GU, TU) ->
        tsize GU GHN TU < S (tsize GX GHN TX) ->
        val_type GU GH GHT GHN vy TU.
    
    Lemma vtp_change_size_aux0: forall G GH GHT1 GHT0 GHN1 GHN0 v T T1 T2 n1 n2,
      val_type G GH (GHT1++[T1]++GHT0) (GHN1++[n1]++GHN0) v T  ->
      (forall TX,
         (has_bounds G GH (GHT1++[T1]++GHT0) (GHN1++[n1]++GHN0) TX) <->
         (has_bounds G GH (GHT1++[T2]++GHT0) (GHN1++[n2]++GHN0) TX)) ->
      vtp G GH  (GHT1++[T2]++GHT0) (GHN1++[n2]++GHN0) v T.
    Proof. admit. Qed.

    Hint Unfold has_bounds. 
    
    Lemma vtp_change_size_aux: forall G GH GHT1 GHT0 GHN1 GHN0 v T T1 T2 n1 n2,
      val_type G GH (GHT1++[T1]++GHT0) (GHN1++[n1]++GHN0) v T  ->
      (forall TX,
         (has_bounds G GH (GHT1++[T1]++GHT0) (GHN1++[n1]++GHN0) TX) <->
         (has_bounds G GH (GHT1++[T2]++GHT0) (GHN1++[n2]++GHN0)  TX)) ->
      vtp G GH (GHT1++[T2]++GHT0) (GHN1++[n2]++GHN0) v T.
    Proof.
      intros. destruct T; rewrite val_type_unfold in H.
      - admit.
      - admit.
      - destruct v; try contradiction.
        ev. eapply vv. rewrite val_type_unfold. split. auto. split. auto.
        intros. destruct (H2 vx jj).
        { eapply unvv. eapply vtp_change_size_aux0. eapply H3.
          split. + intros. eapply H0. eapply H7. + intros. specialize (H0 TX). destruct H0. eapply H0. eapply H7. }
        admit. admit.
        intros.
        assert (has_bounds G (GHT1++[T2]++GHT0) T3). eapply H0. repeat eexists. eapply H8.
        destruct H10. 
        specialize (H6 vy H7 GL TL GU TU).
        
        eapply unvv. eapply vtp_change_size_aux0. eapply H6.
        admit. (* cannot use H10 !!! wrong res type TU1 vs TU2 *)
      ...
      - admit. 
    Qed.
    Lemma vtp_change_size: forall G GH GHT GHN0 v T n1 n2,
      val_type G GH GHT (n1::GHN0) v T  ->
      vtp G GH GHT (n2::GHN0) v T.
    Proof.
      admit.
    Qed.
    eapply vtp_change_size in VT.
    
    eapply IHn. apply unvv. eapply VT. eapply H10. 
    ...
    { case_eq (beq_nat x1 (length GH)); intros E.
      + rewrite E in H15. inversion H15. subst HX TX0.
        exists vx. exists jj. 
        split. reflexivity.
        assert (vtp H2 (jj :: GH) (GHT) (GHN) vx T4) as VXE. eapply valtp_extendH. eapply VX0.
    ...
    + rewrite E in H14. inversion H14. subst HX TX. 
      exists vx. exists jj. 
      split. reflexivity.
      assert (vtp H2 (jj :: GH) f vx T4) as VXE. eapply valtp_extendH. eapply VX0.
      split. eapply unvv. eapply VXE.
      split. 
      ...
      specialize (STJ vy H15).
       assert (exists GL2 TL2 GU2 TU2 n,
                ...
                stp2 false false GU2 TU2 G2 T2 ((H2,T4)::GH1) n /\ n <= ns). {
        eapply inv_bounds. eapply unvv. eapply VXE. eapply H16. eauto. econstructor. }
       ev.
       eapply unvv. eapply IHn. eapply unvv. eapply valtp_extendH. eapply STJ. eapply H18. eapply H19. omega. simpl. rewrite H4. reflexivity.
...
Lemma valtp_widen: forall vf f H1 H2 T1 T2,
  val_type H1 [] f vf T1 ->
  stpd2 true true H1 T1 H2 T2 [] ->
  vtp H2 [] f vf T2.
Proof.
...
Lemma valtp_reg: forall vf f H1 T1,
  val_type H1 [] f vf T1 ->
...
Lemma valtp_extend: forall vx vf f H1 T1,
  val_type H1 [] f vf T1 ->
  vtp (vx::H1) [] f vf T1.
Proof.
...
Lemma inv_vtp_half: forall G v f T GH,
  val_type G [] f v T ->
  exists G0 T0, val_type_stub G0 v T0 /\ closed 0 0 (length G0) T0 /\
             stpd2 false false G0 T0 G T GH.
Proof.
...
Proof.
  ...
  - Case "sel1".
    assert (exists f v, indexr x GX = Some v /\ val_type GX [] f v TX) as A.
    eexists. eapply indexr_safe_ex. eauto. eauto.
    destruct A as [? [? [? VT]]].
  ...
  - Case "selx". eauto.
    assert (exists f v0, indexr x GX = Some v0 /\ val_type GX [] f v0 v) as A.
    eexists. eapply indexr_safe_ex. eauto. eauto. eauto.
    destruct A as [? [? [? ?]]].
    eapply stpd2_selx; eauto.
  - Case "sela1".
...
Lemma invert_abs: forall venv f vf T1 T2,
  val_type venv [] f vf (TAll T1 T2) ->
  exists env TX y,
    vf = (vabs env TX y) /\ 
    (closed 0 0 (length venv) T2 -> forall vx : vl,
       val_type venv [] f vx T1 ->
       exists v : vl, tevaln (vx::env) y v /\ val_type venv [] f v T2).
Proof.
  intros. 
  rewrite val_type_unfold in H.   
  ...
  assert (exists (jj:vl->Prop), forall vy, jj vy ->
        forall (GL : venv) (TL : ty) (GU : venv) (TU : ty),
        bounds venv0 [] T1 (GL, TL) (GU, TU) -> val_type GU [] f vy TU) as F. 
  exists (fun vy => forall (GL : venv) (TL : ty) (GU : venv) (TU : ty),
                              bounds venv0 [] T1 (GL, TL) (GU, TU) -> val_type GU [] f vy TU). intros. eapply H3. eapply H4.

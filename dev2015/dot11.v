...
| t_typ: forall env x T1 T1X,
           fresh env = x ->
           open (TSel x) T1 = T1X ->
           stp ((x,TMem T1X T1X)::env) [] None T1X T1X ->
...
| stp2_sel1: forall G1 G2 TX x T2 GH n1 n2 v,
    ...
    closed 0 0 TX ->
    ptp2 n2 v (base v) TX G2 (TMem TBot T2) GH n1 ->
...
| stp2_sel2: forall G1 G2 TX x T1 GH n1 n2 v,
    ...
    val_type (base v) v TX ->           
    closed 0 0 TX ->
    ptp2 n2 v (base v) TX G1 (TMem T1 TTop) GH n1 ->
...
with ptp2 : nat -> vl -> venv -> ty -> venv ->  ty -> aenv -> nat -> Prop :=
| ptp2_var: forall n2 G1 G2 GH T1 T2 v n1,
           stp2 false false G1 T1 G2 T2 GH n1 ->
           ptp2 n2 v G1 T1 G2 T2 GH (S n1)
| ptp2_unpack: forall G1 G2 G3 GH T1 T2 T2' T3 x v n1 n2 n10,
           ptp2 n10 v G1 T1 G2 (TBind T2') GH n1 ->
           T2 = open (TSel x) T2' ->
           stp2 false true G2 T2 G3 T3 GH n2 ->
           ptp2 (S n10) v G1 T1 G3 T3 GH (S (n1+n2))

(*                
with ptp3 : nat -> vl -> venv -> ty -> venv ->  ty -> aenv -> nat -> Prop :=
| ptp3_var: forall m G1 G2 GH T1 T2 v n1,
           stp2 false false G1 T1 G2 T2 GH n1 ->
           ptp3 0 m v G1 T1 G2 T2 GH (S n1)
| ptp3_unpack: forall m G1 G2 G3 GH T1 T2 T1' T3 x v n1 n2,
           stp2 false false G1 T1 G2 T2 GH n1 ->
           index x G1 = Some v ->
           T1 = open (TSel x) T1' ->
           (* need true (=notrans) here to invert!*)
           ptp3 m v G2 T2 G3 T3 GH n2 ->
           ptp3 m v G1 (TBind T1') G3 T3 GH ( (n1+n2))

| ptp2_var: forall m G1 G2 GH T1 T2 v n1,
           stp2 true true G1 T1 G2 T2 GH n1 ->
...
| ptp2_pack: forall G1 G2 G3 GH T1 T2 T2' T3 x v n1 n2,
           ...
           T2 = open (TSel x) T2' ->
           (* need true (=notrans) here to invert!*)
           stp2 true true G2 (TBind T2') G3 T3 GH n2 ->
...
| ptp2_unpacka: forall x G1 G2 G3 GH T1 T2 T2' T3 n1 n2,
           ...
           T2 = open (TSel x) T2' ->
           stp2 false false G2 T2 G3 T3 GH n2 ->
           ptpa2 x G1 T1 G3 T3 GH (S (n1+n2))
...
Proof.
  ...
  - Case "sel1".
    (* invert bind *)
    assert (forall n', n' < n ->
         forall n0 GX0 TX0 GX1 TX1 m,
           val_type GX0 v TX0 ->
           ptp2 (S m) v GX0 TX0  GX1 TX1 [] n0 -> n0 < n' ->
           exists GX2 TX2 n3, val_type GX2 v TX2 /\
             ptp2 m v GX2 TX2  GX1 TX1 [] n3) as ST. {
      intros n' E. induction n'; intros. subst. omega. 
      inversion H13; subst.
      - SCase "var".
        repeat eexists. eauto. eauto.
      - SCase "unpack".
        inversion H16. (* var, we're at end *)
    }
      
    assert (forall n', n' < n ->
       forall n0 n1 GX1 TX1 m,
         ptp2 false v GX1 TX1 G2 (TMem TBot T2) [] n1 ->
         ptp2 m v (base v) TX  GX1 TX1 [] n0 -> S (n0+n1) < n' ->
         exists n3, ptp3 false v (base v) TX G2 (TMem TBot T2) [] n3) as ST. {
      intros n' E. induction n'; intros. subst. omega. 
      inversion H13; subst.
      - SCase "var".
        admit. (* widen *)
      - SCase "pack".
        inversion H5. subst. admit. (* var *)
        subst.
        inversion H18. subst.
        assert (stp2 false false G5 (open (TSel x0) T2')  G3 T0 [] n1) as SU. admit.
(*
        ptp3 false v G3 T0 G2 (TMem TBot T2) [] n2
        ptp2 m0 v (base v) TX G5 (open (TSel x0) T2') [] n4
 *)
      ...
      - SCase "unpack".
(*
  H18 : stp2 false false G5 (open (TSel x0) T2') GX1 TX1 [] n5
  H5 : ptp3 false v GX1 TX1 G2 (TMem TBot T2) [] n3
*)      
        assert (ptp3 false v G5 (TBind T2') G2 (TMem TBot T2) [] (n5+n3)) as IN.
        eapply ptp3_unpack. eauto. eauto. eauto. eauto.
        eapply IHn'. omega. eapply IN. eapply H15. omega.


(* DONE *)

        
        inversion H5. subst. admit. (* contradiction *)
        subst.

        (*
  H6 : ptp3 false v (base v) TX G3 T0 [] n1
  H7 : stp2 false false G3 T0 GX1 (open (TSel x1) T3') [] n2
  H15 : stp2 false true GX1 (TBind T3') G5 (TBind T2') [] n4
  H18 : ptp2 false v G5 (open (TSel x0) T2') G2 (TMem TBot T2) [] n5

  H6 : ptp3 false v (base v) TX G3 T0 [] n1
  H7 : stp2 false false G3 T0  G5 (open (TSel x0) T2')
  H18 : ptp2 false v G5 (open (TSel x0) T2') G2 (TMem TBot T2) [] n5

             *)
        
        
        assert (val_type G5 v (TBind T2')). eapply valtp_widen. eapply H5. admit.
        inversion H6. admit. admit. admit. admit.
        subst.
...
Proof.
  ...
  - Case "sela1".
    ...
    + SCase "x > 0".
      ev. subst.
      eapply stp2_sela1. eauto. eapply IHn; eauto. omega. eauto. eapply compat_mem_fwd2. eauto.

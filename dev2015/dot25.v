...
Definition base_type v := match v with
| vbool _ => TBool
| vobj _ _ ((m, dmem T1)::_) => TMem m T1 T1 (* FIXME !!! *)
| _ => TTop
end.

Definition base_env v := match v with
| vobj venv f ds => ((f, v)::venv)
| _ => nil
end.
...
| v_var: forall venv x v n venv3 T3,
    index x venv = Some v ->
    val_type (base_env v) v (base_type v) 1 ->
    (exists n, stp2 0 true venv (TVar (varF x)) venv3 T3 [] n) ->
    val_type venv3 v T3 (S (S n))
.
  ...
  - Case "selab1". 
    ...
    + assert (S x0 = x0 + 1) as EQ by omega.
      assert ((splice (length G0) (TBind (TMem m TBot T2)))=(TBind (TMem m TBot T2))) as A. {
        eapply closed_splice_idem. eapply closed_upgrade_free. eauto. eauto. 
      }
...
Proof.
  ...
  - Case "sela1".
    eapply stp_sela1; try eassumption.
    eapply indexr_extend. eassumption.
    instantiate (1:=(x,v1)::GU). simpl. rewrite H2. reflexivity.
  - Case "sela2".
    eapply stp_sela2; try eassumption.
    eapply indexr_extend. eassumption.
    instantiate (1:=(x,v1)::GU). simpl. rewrite H2. reflexivity.
  - Case "selab1".
    assert (closed 0 (S x0) TX). eassumption.
...
Proof.
  intros. induction H.
  eauto; econstructor; eauto; eapply sstpd2_extend2; eauto.
  eauto; econstructor; eauto; eapply sstpd2_extend2; eauto.
  eauto; econstructor; eauto; eapply sstpd2_extend2; eauto.
  eapply v_var; eauto; eapply sstpd2_extend2; eauto. 
Qed.
...
Proof.
  ...
  - Case "mem". subst. inversion H1.
    ...
    + SCase "or22". subst. eexists. eapply stp2_or22; eauto.
  - Case "svar1". subst.
    assert (sstpd2 true (base_env v) (base_type v) G3 T3 []). {
      eapply IHn. eassumption. omega.
      eexists. eapply H1. 
    }
    repeat eu.
...
(*  
  ...
  - (* var *) subst. 
              ...
              assert ( val_type H2 vf T2 1). eapply IHni. omega. eauto. eexists. eauto.
              admit. (* downgrade n *)
  - (* varx *) subst. eapply v_var. rewrite H5 in H3. inversion H3. subst. eauto. 
  - (* sel *) subst. 

  eapply v_var; eauto; eapply sstpd2_trans; eauto.*)
Qed.
...
Lemma invert_typb: forall n, can_subst n -> forall venv vx x l G2 TX T1 T2 n1,
  index x G2 = Some vx \/ closed 0 0 (TMem l T1 T2) -> 
  ...
  - Case "vara1".
    ...
    + SCase "x = 0".
      ...
      * subst. simpl. inversion H10. subst.
        ...
        eapply IHn in H5. destruct H5.
        eexists.
        eapply stp2_var1. eauto. eauto. eapply closed_subst. eauto. eauto. 
        eauto.
        eauto.
        omega. eauto. eauto. eauto. eauto. eauto. eauto. eauto. eauto. 
        omega. eauto. eauto. eauto. eauto. eauto. eauto. eauto. eauto.
...
(*        eapply IHn in H6. destruct H6.
    ...
    + SCase "x > 0".
      ...
      { destruct H13.
      repeat ev. inversion H14. subst V. left. repeat eexists; eauto. eauto. simpl. subst x1. eauto.
  ...
  - Case "selab2". admit. (*
    ...
    + SCase "x = 0". ev. subst.
     ...
     (subst (varF x2) (open (varH 0) T2))) as CPH. {
        ...
        assert (val_type venv0 x0 (open (varF x2) T2) 1). { (* this looks fishy: how can we also end up with valtp 1 ?!? *)
  ...
  - Case "sela2". admit. (*
    ...
    + SCase "x = 0".
      ...
      * subst. simpl. inversion H10. subst.
        ...
        eapply IHn in H5. destruct H5.
        eexists.
        assert (val_type GXX x1 (subst TXX' TXX) 1). { (* this looks fishy: how can we also end up with valtp 1 ?!? *)
          induction ni. apply VS.
          eapply IHni0. eauto. eauto.
        }
  ...
  - Case "transf".
    ...
    {
      eexists.
      unfold compat. simpl. left. exists (fresh G3). exists V. exists nv.

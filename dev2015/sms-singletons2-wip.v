...
| stp2_strong_sel1: forall m GH G1 TX x n1,
    index x G1 = Some (vty TX) ->
    (* stp2 m false [] G1 TX T2 n1 -> *)
    stp2 m true GH G1 (TSel (TVar true x)) TX (S n1)
| stp2_strong_sel2: forall m GH G1 TX x n1,
    index x G1 = Some (vty TX) ->
    (* stp2 m false [] G1 T1 TX n1 -> *)
    stp2 m false GH G1 TX (TSel (TVar true x)) (S n1)
...
| stp2_sel1: forall m GH G1 T2 x n1,
    htp  m true GH G1 x (TMem TBot T2) n1 ->
    stp2 m false GH G1 (TSel (TVar false x)) T2 (S n1)

| stp2_sel2: forall m GH G1 T1 x n1,
    htp  m true GH G1 x (TMem T1 TTop) n1 ->
    stp2 m false GH G1 T1 (TSel (TVar false x)) (S n1)
...
Proof.
  ...
  - admit. (* eapply closed_upgrade_gh. eapply IHS1. eapply H2. omega. simpl. omega.  *)
  - admit. (* econstructor. econstructor. eapply IHH1. eauto. omega. *)
  ...
  - admit. (* eapply closed_upgrade_gh. eapply IHS2. eapply H2. omega. simpl. omega.  *)
  - admit. (* econstructor. econstructor. eapply index_max. eauto. *)
  - eapply closed_upgrade_gh. eapply IHH2 in H1. inversion H1. eauto. omega. simpl. omega.
...
Proof.
  ...
  assert (forall ni n2, forall m GH T2 xi,
    htp m true (GH ++ [TX]) G1 xi T2 n2 -> xi <> 0 -> n2 < ni -> ni < S n ->
    htpd m true (map (substt x) GH) G1 (xi-1) (substt x T2)) as htp_subst_narrow02. {
      ...
      + (* sub *) subst.
        ...
        assert (stpd2 m true (map (substt x) GL0) G1 (substt x T3) (substt x T0)) as BB.
  ...
  assert (forall ni n2, forall m T0 T2,
    htp m true (T0 :: GH ++ [TX]) G1 (length (GH ++ [TX])) T2 n2 -> n2 < ni -> ni < S n ->
    htpd m true (map (substt x) (T0::GH)) G1 (length GH) (substt x T2)) as htp_subst_narrow0. {
      intros.
...
*)
  ...
  - Case "sel1". subst. (* invert htp to vtp and create strong_sel node *)
    ...
    + assert (x0 = 0). eapply beq_nat_true_iff. eauto. subst x0.
      ...
      repeat eexists. eapply stp2_transf. eapply stp2_strong_sel1. eauto. 
      instantiate (1:=n2). instantiate (1:=0). admit. (* FIXME: m2 vs ms *) (* FIXME: extend GH *)
    + assert (x0 <> 0). eapply beq_nat_false_iff. eauto.
  ...
  - Case "sel2". subst. (* invert htp to vtp and create strong_sel node *)
    ...
    + assert (x0 = 0). eapply beq_nat_true_iff. eauto. subst x0.
      ...
      destruct A as [? A]. eu. inversion A. subst. 
      repeat eexists. eapply stp2_strong_sel2. eauto. unfold substt. admit. (* FIXME: trans shuffle, extend GH *)
  ...
  - Case "transf". 
    assert (stpd2 m true (map (substt x) GH) G1 (substt x T1) (substt x T3)).
    eapply IHn; eauto. omega.
...
Lemma stp2_trans: forall l, forall n, forall k, forall b m1 m2 G1 x T2 T3 n1 n2,
  vtp m1 G1 x T2 n1 -> 
  stp2 m2 b [] G1 T2 T3 n2 ->
  m1 < l -> n2 < n -> n1 < k -> 
  (match b with | true => vtpdd m1 G1 x T3 | false => (exists mx, vtpd mx G1 x T3) end).
Proof.
  ...
  - Case "top". inversion H0; subst; invty.
    + SCase "top". eauto. (* repeat eexists; eauto. left. eauto. *)
    (* + SCase "ssel2".
      assert (exists mx,vtpd mx G1 x TX). eapply IHn with (b:=false); eauto. omega. 
      ev. eu. repeat eexists. eapply vtp_sel. eauto. eauto. eauto. omega. 
    + SCase "sel2".
...
(*    + SCase "ssel2". 
  ...
  - Case "mem". inversion H0; subst; invty.
    + SCase "top". repeat eexists. eapply vtp_top. eapply index_max. eauto. eauto. 
    + SCase "mem". admit. (* PROBLEM: T < x.A ??? invty. subst.
...
(*    + SCase "sel2". 
    ...
    + SCase "bind1".
      ...
      assert (vtpdd m G1 x (substt x T3)) as BB. {
        ...
        { intros. eapply IHl with (b:=true). eauto. eauto. omega. eauto. eauto. }
      }
      rewrite R1 in BB. 
      eu. repeat eexists. eauto. omega. 
    + SCase "bindx".
  ...
  - Case "ssel2". subst. inversion H0; subst; invty.
    + SCase "top". repeat eexists. eapply vtp_top. eapply vtp_closed1. eauto. eauto. 
    + SCase "ssel1". index_subst. repeat eexists. eauto. eauto. 


      (* index_subst. eapply IHn with (b:=false). eauto. eauto. eauto. omega. eauto. *)
(*    + SCase "ssel2". 
      assert (vtpdd m1 G1 x TX0). eapply IHn; eauto. omega. 
      eu. repeat eexists. eapply vtp_sel. eauto. eauto. eauto.
    + SCase "sel1".
  ...
  - Case "ssel2". 
      repeat eexists. eapply vtp_sel. eauto. eauto.
  - Case "sel1". subst. 
    assert (closed (length ([]:tenv)) (length G1) 0 (TSel (TVar false x0))).
      eapply stpd2_closed2. eauto.
      simpl in H5. inversion H5. inversion H10. omega.
  - Case "sel2". subst. 
    assert (closed (length ([]:tenv)) (length G1) 0 (TSel (TVar false x0))).
      eapply stpd2_closed2. eauto.
      simpl in H5. inversion H5. inversion H10. omega.
  ...
  - Case "wrapf".
    assert (vtpdd m1 G1 x T3). eapply IHn with (b:=true). eapply H. eauto. eauto. omega. eauto.
    eu. repeat eexists. eauto. 
  - Case "transf". subst.
    assert (vtpdd m1 G1 x T0) as LHS. eapply IHn with (b:=true). eauto. eauto. eauto. omega. eauto.
    eu.
    eapply IHn with (b:=false). eauto. eauto. omega. omega. eauto. 
  }

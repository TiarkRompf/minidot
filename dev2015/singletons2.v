...
Inductive stp2: nat -> bool -> aenv -> venv -> ty -> venv -> ty -> nat -> Prop :=
| stp2_bot: forall m GH G1 G2 T n1,
    closed (length GH) (length G2) 0  T ->
    stp2 m true GH G1 TBot G2 T (S n1)
| stp2_top: forall m GH G1 G2 T n1,
    closed (length GH) (length G1) 0 T ->
    stp2 m true GH G1 T G2 TTop (S n1)
| stp2_bool: forall m GH G1 G2 n1,
    stp2 m true GH G1 TBool G2 TBool (S n1)
| stp2_fun: forall m GH G1 G2 T1 T2 T3 T4 n1 n2,
    stp2 m false GH G2 T3 G1 T1 n1 ->
    stp2 m false GH G1 T2 G2 T4 n2 ->
    stp2 m true GH G1 (TFun T1 T2) G2 (TFun T3 T4) (S (n1+n2))

| stp2_varx: forall m GH G1 G2 x1 x2 v n1,
    index x1 G1 = Some v ->
    index x2 G2 = Some v ->
    stp2 m true GH G1 (TVar true x1) G2 (TVar true x2) (S n1)
| stp2_var1: forall m GH G1 GX TX G2 x T2 v n1,
    index x G1 = Some v ->
    val_type0 GX v TX -> (* slack for: val_type G2 v T2 *)
    stp2 m true GH GX TX G2 T2 n1 ->
    stp2 m true GH G1 (TVar true x) G2 T2 (S n1)
| stp2_var1b: forall m GH G1 GX TX G2 x x2 T2 v n1 n2,
    index x G1 = Some v ->
    index x2 G2 = Some v ->            
    val_type0 GX v TX -> (* slack for: val_type G2 v T2 *)
    stp2 m true ((G1,(TVar true x))::GH) GX TX G2 (open 0 (TVar false (length GH)) T2) n1 ->
    stp2 m true GH GX TX G2 (open 0 (TVar true x2) T2) n2 -> 
    stp2 m true GH G1 (TVar true x) G2 (TBind T2) (S (n1+n2))

| stp2_varax: forall m GH G1 G2 x n1,
    x < length GH ->
    stp2 m true GH G1 (TVar false x) G2 (TVar false x) (S n1)
| stp2_varay: forall m GH G1 G2 GX x1 x2 n1,
    index x1 GH = Some (GX, (TVar false x2)) ->
    x2 < length GH ->
    stp2 m true GH G1 (TVar false x2) G2 (TVar false x1) (S n1)
| stp2_vara1: forall m GH G1 GX TX G2 x T2 n1,
    index x GH = Some (GX, TX) ->
    stp2 m true GH GX TX G2 T2 n1 ->
    stp2 m true GH G1 (TVar false x) G2 T2 (S n1)


| stp2_mem: forall m GH G1 G2 T1 T2 T3 T4 n1 n2,
    stp2 m false GH G2 T3 G1 T1 n2 ->
    stp2 m true GH G1 T2 G2 T4 n1 ->
    stp2 m true GH G1 (TMem T1 T2) G2 (TMem T3 T4) (S (n1+n2))
| stp2_sel: forall m GH G1 G2 T1 T2 n1 n2,
    stp2 m false GH G2 T2 G1 T1 n2 ->
    stp2 m true GH G1 T1 G2 T2 n1 ->
    stp2 m true GH G1 (TSel T1) G2 (TSel T2) (S (n1+n2))
| stp2_red: forall m GH G1 G2 T1 T2 n1,
    stp2 m true GH G1 T1 G2 (TMem TBot T2) n1 ->
    stp2 m true GH G1 (TSel T1) G2 T2 (S n1)
| stp2_red2: forall m GH G1 G2 GX TX T1 T2 GY T0 n0 n1 n2 n3,
(* long form of:
    stp2 true GY (TVar y) G2 T2 n0 ->
    stp2 true G2 T2 G1 (TMem T1 TTop) n1 -> *)
    real T0 ->
    stp2 m true GH GY T0 G2 T2 n0 ->
    stp2 m false GH G2 T2 GX (TMem TX TTop) n1 ->
    stp2 m true GH GY T0 GX (TMem TX TTop) n3 ->
    stp2 m false GH G1 T1 GX TX n2 ->
    stp2 m true GH G1 T1 G2 (TSel T2) (S (n0+n1+n2+n3))

(* 
| stp2_bindI: forall G1 GX TX G2 x T2 v n1,
    index x G1 = Some v ->
    val_type0 GX v TX -> (* slack for: val_type G2 v T2 *)
    stp2 true GX TX G2 (open 0 (TVar x) T2) n1 ->
    stp2 true G1 (TVar x) G2 (TBind T2) (S n1)
*)

(*         
| stp2_bindE: forall G1 G2 T2 x n1,
    stp2 true G1 (TVar x) G2 (TBind T2) n1 ->
    stp2 true G1 (TVar x) G2 (open 0 (TVar x) T2) (S n1)
*)

(*
         
| stp2_bind2: forall m GH G1 G2 T2 x n1,
    (* issue here: we want x.this < { z => T2 }
       and z:x.this |- *)
                
    stp2 m true GH G1 (TVar false x) G2 (open 0 (TVar false x) T2) n1 ->
    stp2 m true GH G1 (TVar false x) G2 (TBind T2) (S n1)
         
| stp2_bind1: forall m GH G1 G2 T1 T2 n1,
    closed (length GH) (length G2) 0 T2 ->
    stp2 m true ((G1,(open 0 (TVar false (length GH)) T1))::GH) G1 (TVar false (length GH)) G2 T2 n1 ->
    stp2 m true GH G1 (TBind T1) G2 T2 (S n1)
...
| stp2_wrapf: forall m GH G1 G2 T1 T2 n1,
    stp2 m true GH G1 T1 G2 T2 n1 ->
    stp2 m false GH G1 T1 G2 T2 (S n1)
| stp2_transf: forall m GH G1 G2 G3 T1 T2 T3 n1 n2,
    stp2 m true GH G1 T1 G2 T2 n1 ->
    stp2 m false GH G2 T2 G3 T3 n2 ->
    stp2 m false GH G1 T1 G3 T3 (S (n1+n2))
...
| v_sub: forall G1 T1 G2 T2 v,
    val_type0 G1 v T1 ->
    (exists n, stp2 0 true [] G1 T1 G2 T2 n) ->
    val_type G2 v T2
.
              


Definition stpd2 m b GH G1 T1 G2 T2 := exists n, stp2 m b GH G1 T1 G2 T2 n.

Hint Constructors stp2.

Ltac ep := match goal with
             | [ |- stp2 ?M ?B ?GH ?G1 ?T1 ?G2 ?T2 ?N ] => assert (exists (n:nat), stp2 M B GH G1 T1 G2 T2 n) as EEX
           end.
...
Lemma stpd2_bot: forall m GH G1 G2 T,
    closed (length GH) (length G2) 0 T ->
    stpd2 m true GH G1 TBot G2 T.
Proof. intros. exists 1. eauto. Qed.
Lemma stpd2_top: forall m GH G1 G2 T,
    closed (length GH) (length G1) 0 T ->
    stpd2 m true GH G1 T G2 TTop.
Proof. intros. exists 1. eauto. Qed.
Lemma stpd2_bool: forall m GH G1 G2,
    stpd2 m true GH G1 TBool G2 TBool.
Proof. intros. exists 1. eauto. Qed.
Lemma stpd2_fun: forall m GH G1 G2 T1 T2 T3 T4,
    stpd2 m false GH G2 T3 G1 T1 ->
    stpd2 m false GH G1 T2 G2 T4 ->
    stpd2 m true  GH G1 (TFun T1 T2) G2 (TFun T3 T4).
Proof. intros. repeat eu. eexists. eauto. Qed.
Lemma stpd2_varx: forall m GH G1 G2 x1 x2 v,
    index x1 G1 = Some v ->
    index x2 G2 = Some v ->
    stpd2 m true GH G1 (TVar true x1) G2 (TVar true x2).
Proof. intros. repeat eu. exists 1. eauto. Qed.
Lemma stpd2_var1: forall m GH G1 GX TX G2 x T2 v,
    index x G1 = Some v ->
    val_type0 GX v TX -> (* slack for: val_type G2 v T2 *)
    stpd2 m true GH GX TX G2 T2 ->
    stpd2 m true GH G1 (TVar true x) G2 T2.
Proof. intros. repeat eu. eexists. eauto. Qed.
...
Proof. intros. repeat eu. eexists. eauto. Qed.*)
Lemma stpd2_mem: forall m GH G1 G2 T1 T2 T3 T4,
    stpd2 m false GH G2 T3 G1 T1 ->
    stpd2 m true  GH G1 T2 G2 T4 ->
    stpd2 m true  GH G1 (TMem T1 T2) G2 (TMem T3 T4).
Proof. intros. repeat eu. eexists. eauto. Qed.
Lemma stpd2_sel: forall m GH G1 G2 T1 T2,
    stpd2 m false GH G2 T2 G1 T1 ->
    stpd2 m true  GH G1 T1 G2 T2 ->
    stpd2 m true  GH G1 (TSel T1) G2 (TSel T2).
Proof. intros. repeat eu. eexists. eauto. Qed.
Lemma stpd2_red: forall m GH G1 G2 T1 T2,
    stpd2 m true GH G1 T1 G2 (TMem TBot T2) ->
    stpd2 m true GH G1 (TSel T1) G2 T2.
Proof. intros. repeat eu. eexists. eauto. Qed.
Lemma stpd2_red2: forall m GH G1 G2 GX TX T1 T2 GY T0,
    real T0 ->
    stpd2 m true GH GY T0 G2 T2 ->
    stpd2 m false GH G2 T2 GX (TMem TX TTop) ->
    stpd2 m true GH GY T0 GX (TMem TX TTop) ->
    stpd2 m false GH G1 T1 GX TX ->
    stpd2 m true GH G1 T1 G2 (TSel T2).
Proof. intros. repeat eu. eexists. eauto. Qed.
...
Lemma stpd2_bind1: forall m GH G1 G2 T1 T2,
    closed (length GH) (length G2) 0 T2 ->
...
Lemma stpd2_wrapf: forall m GH G1 G2 T1 T2,
    stpd2 m true GH G1 T1 G2 T2 ->
    stpd2 m false GH G1 T1 G2 T2.
Proof. intros. repeat eu. eexists. eauto. Qed.
Lemma stpd2_transf: forall m GH G1 G2 G3 T1 T2 T3,
    stpd2 m true GH G1 T1 G2 T2 ->
    stpd2 m false GH G2 T2 G3 T3 ->
    stpd2 m false GH G1 T1 G3 T3.
Proof. intros. repeat eu. eexists. eauto. Qed.
...
Lemma stpd2_closed : forall m b GH G1 G2 T1 T2,
                      stpd2 m b GH G1 T1 G2 T2 ->
...
Lemma stpd2_closed1 : forall m b GH G1 G2 T1 T2,
                      stpd2 m b GH G1 T1 G2 T2 ->
                      closed (length GH) (length G1) 0 T1.
Proof. intros. eapply (stpd2_closed m b GH G1 G2); eauto. Qed.

Lemma stpd2_closed2 : forall m b GH G1 G2 T1 T2,
                      stpd2 m b GH G1 T1 G2 T2 ->
                      closed (length GH) (length G2) 0 T2.
Proof. intros. eapply (stpd2_closed m b GH G1 G2); eauto. Qed.
...
Proof.
  ...
  - Case "bind".
    eapply stpd2_bind1. eauto. exists 2.
    eapply stp2_bind2.
    simpl. eauto.
...
Lemma stpd2_reg1 : forall m b GH G1 G2 T1 T2,
                      stpd2 m b GH G1 T1 G2 T2 ->
                      stpd2 m true GH G1 T1 G1 T1.
Proof.
  intros. eapply stpd2_refl. eapply (stpd2_closed m b GH G1 G2). eauto.
Qed.

Lemma stpd2_reg2 : forall m b GH G1 G2 T1 T2,
                      stpd2 m b GH G1 T1 G2 T2 ->
                      stpd2 m true GH G2 T2 G2 T2.
Proof.
  intros. eapply stpd2_refl. eapply (stpd2_closed m b GH G1 G2). eauto.
Qed.
...
Lemma invert_mem1: forall n, forall venv m GH vf T1 T2 GX TX n1,
  val_type0 GX vf TX -> stp2 m true GH GX TX venv (TMem T1 T2) n1 -> n1 < n ->
  exists env T3 n2 n3,
    vf = (vty env T3) /\
    (n2 + n3) < n /\
    stp2 m false GH venv T1 env T3 n2 /\
    stp2 m true GH env T3 venv T2 n3.
Proof.
  intros n. induction n; intros. solve by inversion.
  inversion H; subst. 
  - Case "bool". solve by inversion.
...
Lemma stpd2_trans_axiom_aux: forall n, forall m GH G1 G2 G3 T1 T2 T3 n1,
  stp2 m false GH G1 T1 G2 T2 n1 -> n1 < n ->
  stpd2 m false GH G2 T2 G3 T3 ->
  stpd2 m false GH G1 T1 G3 T3.
Proof.
...
Lemma stp2_trans_axiom: forall m b GH G1 G2 G3 T1 T2 T3,
  stpd2 m b GH G1 T1 G2 T2 -> 
  stpd2 m false GH G2 T2 G3 T3 ->
  stpd2 m false GH G1 T1 G3 T3.
Proof.
...
Ltac invstp_var := match goal with
  | H1: stp2 _ true _ _ TBot        _ (TVar _ _) _ |- _ => inversion H1
  | H1: stp2 _ true _ _ TTop        _ (TVar _ _) _ |- _ => inversion H1
  | H1: stp2 _ true _ _ TBool       _ (TVar _ _) _ |- _ => inversion H1
...
Lemma stp2_subst: forall m b G1 G2 T1 T2 GX TX n1,
   stp2 m b [(GX,TX)] G1 T1 G2 T2 n1 ->
   stpd2 m b nil G1 T1 G2 T2.
Proof.
  admit.
Qed.


Lemma stp2_narrow: forall m GX1 GX2 TX1 TX2 G1 G2 T1 T2 n1 n2,
  stp2 m true [(GX1,TX1)] GX1 TX1 GX2 TX2 n1 ->
  stp2 m true [(GX2,TX2)] G1 T1 G2 T2 n2 -> 
  stpd2 m true [(GX1,TX1)] G1 T1 G2 T2.
Proof.
  admit.
Qed.
...
Lemma stp2_trans: forall n, forall m b G1 G2 G3 T1 T2 T3 n1 n2,
  stp2 m b nil G1 T1 G2 T2 n1 -> 
  stp2 m true nil G2 T2 G3 T3 n2 ->
  n1 < n ->
  stpd2 m true nil G1 T1 G3 T3.
Proof.
  intros n. induction n; intros. solve by inversion.
  inversion H.
...
(*    + SCase "bind2". invstp_var. *)
  ...
  - Case "varx". inversion H0; subst; invty.
    + SCase "top". eapply stpd2_top; eauto. eapply stpd2_closed1; eauto.
    + SCase "varx". invty. subst. index_subst. eapply stpd2_varx; eauto.
    + SCase "var1". invty. subst. index_subst. eapply stpd2_var1; eauto.
    + SCase "var1b". admit. (* invty. subst. index_subst. eexists. eapply stp2_var1b; eauto. *)
    + SCase "red2". eapply stpd2_red2. eauto. eauto. eauto. eauto. eapply stp2_trans_axiom; eauto.
  - Case "var1".
    eapply stpd2_var1; eauto. eapply IHn; eauto. omega.
  - Case "var1b". inversion H0; subst; invty.
    + SCase "top". eapply stpd2_top; eauto. eapply stpd2_closed; eauto.
    + SCase "red2". eapply stpd2_red2. eauto. eauto. eauto. eauto. eapply stp2_trans_axiom; eauto.
    + SCase "bind1".
      subst.
      assert (stpd2 m true [(G1, TVar true x)] G2
                    (TVar false (length ([]:aenv))) G3 T3) as NRW.
      eapply stp2_narrow. eapply stp2_var1. eauto. eauto. eauto. eauto. 

      assert (stpd2 m true [(G1, TVar true x)] G2
                    (TVar false (length ([]:aenv))) G3 T3) as NRW.
      
      eapply stpd2_var1. eauto. eauto. eapply IHn. 
      
      (* 
         stp2 m true GH 
          GX TX 
          G2 (open 0 (TVar true x2) T0) n0
         
         stp2 m true ((G2, open 0 (TVar false (length GH)) T0) :: GH) 
          G2 (TVar false (length GH)) 
          G3 T3 n3         
      *)
  ...
  - Case "red2".
    ...
    + SCase "red". clear H. subst. 
      ...
      assert (stp2 m true GH GY T5 G2 T4 n0) as A0. eauto.
      assert (stp2 m true GH GY T5 GX (TMem TX TTop) n5) as A1. eauto.
      assert (stpd2 m true GH GY T5 G3 (TMem TBot T3)) as A2. eapply IHn. eapply A0. eauto. omega. eu.

      destruct H2; inversion A1; inversion A2; subst. (* could pull out into a lemma: invert_real *)
      * SSCase "var".
        index_subst.
        
        assert (exists GX1 TX1 m1 m2, v0 = (vty GX1 TX1) /\ m1 + m2 < S n1 /\
          stp2 m false GH GX TX GX1 TX1 m1 /\ stp2 m true GH GX1 TX1 GX TTop m2) as B1.
        eapply invert_mem1. eapply H7. eauto. eauto. 
        
        assert (exists GX2 TX2 m3 m4, v0 = (vty GX2 TX2) /\ m3 + m4 < S n2 /\
          stp2 m false GH G3 TBot GX2 TX2 m3 /\ stp2 m true GH GX2 TX2 G3 T3 m4) as B2.
        eapply invert_mem1. eapply H18. eauto. eauto.
        ...
        assert (stpd2 m true GH GX TX G3 T3) as S3. eapply IHn. eapply B1. eapply B2. omega. eu.
        assert (stpd2 m true GH G1 T1 G3 T3) as S1. eapply IHn. eauto. eapply S3. omega.
        eapply S1.

      * SSCase "mem".

        assert (stp2 m false GH GX TX GY T n2) as B1. eauto.
        assert (stp2 m true GH GY T G3 T3 n7) as B2. eauto.

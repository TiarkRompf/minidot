...
| stp2_sel1: forall m GH G1 T1 T2 n1,
    (* htp  m false GH G1 x (TMem TBot T2) n1 -> *)
    stp2 m true GH G1 T1 (TMem TBot T2) n1 ->           
    stp2 m true GH G1 (TSel T1) T2 (S n1)

| stp2_sel2: forall m GH G1 b x T1 T2 n1,
    (* htp  m false GH G1 x (TMem T1 TTop) n1 -> *)
    stp2 m true GH G1 (TVar b x) T2 n1 ->
    stp2 m false  GH G1 T2 (TMem T1 TTop) n1 ->
    stp2 m true GH G1 T1 (TSel T2) (S n1)
...
| vtp_sel: forall m m2 G1 x y TY TX n1 n2 n3 n4,
    vtp m G1 x TX n1 ->
    vtp2 G1 y TY n2 ->
    vtp m G1 y (TMem TX TTop) n3 ->
    stp2 m2 false [] G1 TY (TMem TX TTop) n4 ->
    vtp m G1 x (TSel TY) (S (n1+n2+n3+n4))
...
Proof.
  ...
  - admit. (* econstructor. eapply index_extend. eauto. eapply IHn. eauto. omega. *)
  (* htp *)
...
Proof.
  ...
  - econstructor. admit. (* econstructor. eapply IHH1. eauto. omega. *)
  - admit. (* eapply closed_upgrade_gh. eapply IHH2 in H1. inversion H1. eauto. omega. simpl. omega. *)
  - econstructor. eauto.
  - econstructor. eauto.
...
*)
  ...
  - Case "sel1". subst. (* invert htp to vtp and create strong_sel node *)
    assert (stpd2 m2 true (map (substt x) GH) G1 (substt x T0) (substt x (TMem TBot T2))). eapply IHn. eauto. eauto. omega. eauto.
    eu. eexists. eapply stp2_sel1. eauto. 
(*
    case_eq (beq_nat x0 0); intros E.
    + assert (x0 = 0). eapply beq_nat_true_iff. eauto. subst x0.
 ...
 *)
  - Case "sel2". subst. (* invert htp to vtp and create strong_sel node *)
    admit. (* case_eq (beq_nat x0 0); intros E.
    + assert (x0 = 0). eapply beq_nat_true_iff. eauto. subst x0.
...
Proof.
  ...
  - Case "top". inversion H0; subst; invty.
    ...
    + SCase "ssel2".
      ...
      (* PROBLEM: no way to assign (TVar true x) as type *)
      admit. (* eu. repeat eexists. eapply vtp_sel. eauto. eauto. eauto. *)
    + SCase "sel2".
      subst.
      inversion H10. 
      * (* top PROBLEM *) subst.
      assert (vtpdd m1 G1 x (TMem TTop TTop)). eapply IHn; eauto. omega.
      eu. repeat eexists. eapply vtp_sel. eapply H. eapply vtp2_down. eauto.
      assert (x0 = m1). admit. subst x0.
      eauto. eauto. eauto.
      * (* varx PROBLEM *) subst. repeat eexists. eapply vtp_sel. eauto.
        eapply vtp2_refl. eauto. eauto. eauto. 
      * subst. repeat eexists. eapply vtp_sel. eauto. eauto. eauto. eauto.
      * admit. (* closed subst. repeat eexists. eapply vtp_sel. eauto. eauto. eauto. eauto.*)
      * admit. (* narrowX subst. repeat eexists. eapply vtp_sel. eauto. *)
      * (* strong sel *) subst. admit.
      * (* sel *) subst. admit.
  ...
  - Case "ssel2". subst. inversion H0; subst; invty.
    ...
    + SCase "ssel2". 
      ...
      eu. repeat eexists. eapply vtp_sel. eauto. eauto. eauto.
    + SCase "sel1".
      (* make crucial use of stp2 true here (no-trans! ) *)
      inversion H8. inversion H9. inversion H12. subst. index_subst. 
      eapply IHn. clear H. eauto. eauto. omega. omega. eauto. 

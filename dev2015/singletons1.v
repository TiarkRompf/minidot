...
Inductive closed: nat -> nat -> ty -> Prop :=
| cl_bot: forall j k,
    closed j k TBot
| cl_top: forall j k,
    closed j k TTop
| cl_bool: forall j k,
    closed j k TBool
| cl_fun: forall j k T1 T2,
    closed j k T1 ->
    closed j k T2 ->
    closed j k (TFun T1 T2)
| cl_var: forall j k x,
    j > x ->
    closed j k (TVar x)
| cl_varB: forall j k x,
    k > x ->
    closed j k (TVarB x)
| cl_mem: forall j k T1 T2,
    closed j k T1 ->
    closed j k T2 ->        
    closed j k (TMem T1 T2)
| cl_sel: forall j k T1,
    closed j k T1 ->
    closed j k (TSel T1)
| cl_bind: forall j k T1,
    closed j (S k) T1 ->
    closed j k (TBind T1)         
.
...
Fixpoint open (k: nat) (u: ty) (T: ty) { struct T }: ty :=
  match T with
    | TVar x => TVar x (* free var remains free. functional, so we can't check for conflict *)
...
| t_abs: forall env y T1 T2,
    ...
    closed (length env) 0 (TFun T1 T2) ->       
...
| stp2_varx: forall m GH G1 G2 x1 x2 v n1,
    index x1 G1 = Some v ->
    index x2 G2 = Some v ->
    stp2 m true GH G1 (TVar x1) G2 (TVar x2) (S n1)
| stp2_var1: forall m GH G1 GX TX G2 x T2 v n1,
    index x G1 = Some v ->
    val_type0 GX v TX -> (* slack for: val_type G2 v T2 *)
    stp2 m true GH GX TX G2 T2 n1 ->
    stp2 m true GH G1 (TVar x) G2 T2 (S n1)
| stp2_var1b: forall m GH G1 GX TX G2 x x2 T2 v n1,
    index x G1 = Some v ->
    index x2 G2 = Some v ->            
    val_type0 GX v TX -> (* slack for: val_type G2 v T2 *)
    stp2 m true GH GX TX G2 (open 0 (TVar x2) T2) n1 ->
    stp2 m true GH G1 (TVar x) G2 (TBind T2) (S n1)
...
| stp2_bind2: forall m GH G1 G2 T1 T2 v n1,
    (* issue here: we want x.this < { z => T2 }
       and z:x.this |- *)
                
    closed (length G1) 0 T1 ->
    val_type0 G1 v T1 ->
    stp2 m true GH G1 T1 (v::G2) (TVar (length G2)) n1 ->
    stp2 m true GH G1 T1 (v::G2) (open 0 (TVar (length G2)) T2) n1 ->
    stp2 m true GH G1 T1 G2 (TBind T2) (S n1)
| stp2_bind1: forall m GH G1 G2 T1 T2 v n1,
    closed (length G2) 0 T2 ->
    val_type0 G1 v (open 0 (TVar (length G1)) T1) ->
    stp2 m true GH (v::G1) (TVar (length G1)) G2 T2 n1 ->
...
Lemma stpd2_bind1: forall m GH G1 G2 T1 T2 v,
    closed (length G2) 0 T2 ->
    val_type0 G1 v (open 0 (TVar (length G1)) T1) ->
    stpd2 m true GH (v::G1) (TVar (length G1)) G2 T2 ->
...
Lemma closed_extend : forall T X (a:X) k G,
                       closed (length G) k T ->
                       closed (length (a::G)) k T.
Proof.
...
Ltac invstp_var := match goal with
  | H1: stp2 _ true _ _ TBool _ (TVar _) _ |- _ => inversion H1
  | H1: stp2 _ true _ _ (TFun _ _)  _ (TVar _) _ |- _ => inversion H1
  | H1: stp2 _ true _ _ (TMem _ _)  _ (TVar _) _ |- _ => inversion H1
  | H1: val_type0 _ _ _ |- _ => inversion H1
  | _ => idtac
end.

Lemma stp2_trans: forall n, forall m b GH G1 G2 G3 T1 T2 T3 n1 n2,
  stp2 m b GH G1 T1 G2 T2 n1 -> 
  stp2 m true GH G2 T2 G3 T3 n2 ->
  n1 < n ->
...
Proof.
  ...
  - Case "varx". inversion H0; subst; invty.
    ...
    + SCase "var1b". invty. subst. index_subst. eapply stpd2_var1b; eauto. 
    + SCase "red2". eapply stpd2_red2. eauto. eauto. eauto. eauto. eapply stp2_trans_axiom; eauto.
    + SCase "bind2". invstp_var.
  - Case "var1".

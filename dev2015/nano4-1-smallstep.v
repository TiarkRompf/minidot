...
| t_sub: forall sto env t T1 T2,
           has_type sto env t T1->
           stp sto env T1 T2 ->
           has_type sto env t T2

with stp: venv -> tenv -> ty -> ty -> Prop :=
| stp_bool: forall S G,
    stp S G TBool TBool
...
Lemma stp_extend : forall vs ts v1 T1 T2,
                       stp vs ts T1 T2 ->
                       stp (v1::vs) ts T1 T2.
Proof.
...
Lemma stp_extendG : forall vs ts v1 T1 T2,
                       stp vs ts T1 T2 ->
                       stp vs (v1::ts) T1 T2.
Proof.
  intros. induction H; eauto.
...
Proof.
  intros. induction H; eauto.
  econstructor. eapply index_extend; eauto. eauto. econstructor. eauto. eapply stp_extend. eauto.
Qed.

Lemma hastp_extendG : forall vs ts v1 t T,
                       has_type vs ts t T ->
                       has_type vs (v1::ts) t T.
Proof.
  intros. induction H; eauto.
  econstructor. eapply index_extend; eauto. eauto. econstructor. eauto. eauto. admit. admit.
Qed.
...
Lemma hastp_subst: forall sto x T0 env0 t T, 
  has_type sto [] (tloc x) T0 ->
  has_type sto env0 t T ->
  forall env,
    env0 = env ++ [T0] ->
    has_type sto env (subst (tloc x) t) T.
Proof.
  intros. revert env H1. induction H0; intros.
  - eauto.
  - eauto.
  - (* var *) simpl. destruct x0.
    + (* hit *) simpl. subst env. rewrite index_hit0 in H0. inversion H0. subst. eauto.
       remember (tloc x). remember []. induction H; inversion Heqt. 

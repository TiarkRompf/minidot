...
(*
copied from dsubsup-withbind_total5.v
explore different approx metric as in ..8-wip.v

PROBLEMS:
- t_typ case requires vtp (S n) -> vtp n /\ vtp n -> vtp (S n)
- not clear if it solves the var_unpack issue, since the function
  case in val_type uses forall n. thus, bounding n for the value
  itself is not enough to rule out uses of (d (S n)) in the env
  for function args and results.
*)
...
| vsmatch: forall n, vset (S n) -> vset_match n
.                                


Lemma approx {k}: vset (S k) -> vset k. admit. Qed. 


Require Coq.Program.Wf.

Program Fixpoint val_type (env: list vseta) (GH:list vseta) (T:ty) n (dd: vset (S n)) (v:vl) {measure (tsize_flat T)}: Prop :=
  match v,T with
    | vabs env1 T0 y, TAll T1 T2 =>
      closed 0 (length GH) (length env) T1 /\ closed 1 (length GH) (length env) T2 /\
      forall jj vx,
        (forall kx, val_type env GH T1 kx (jj (S kx)) vx) ->
        exists (jj2:vseta) v, tevaln (vx::env1) y v /\ (forall k, val_type env (jj::GH) (open (varH (length GH)) T2) k (jj2 (S k)) v)
    ...
    | vty env1 TX, TMem T1 T2 =>
      ...
      match (vsmatch n dd) with
        ...
        | vsmatch (S n0) dd => forall (dy:vseta) vy, 
                      (val_type env GH T1 (S n0) (dy (S (S n0))) vy -> dd (dy (S n0)) vy) /\
                      (dd (dy (S n0)) vy -> val_type env GH T2 (S n0) (dy (S (S n0))) vy)
      end

    | _, TSel (varF x) =>
      match indexr x env with
        | Some jj => jj (S n) (approx dd) v
        | _ => False
      end
    | _, TSel (varH x) =>
    ...
    | _, TBind T1 =>
      ...
      *)
      ...
      exists jj:vseta, jj (S n) = dd /\ forall n, val_type env (jj::GH) (open (varH (length GH)) T1) n (jj (S n)) v
    | _, TTop => 
      True
    | _,_ =>
      False
...
Lemma val_type_unfold: forall env GH T n dd v, val_type env GH T n dd v =
  match v,T with
    ...
    | vty env1 TX, TMem T1 T2 =>
      closed 0 (length GH) (length env) T1 /\ closed 0 (length GH) (length env) T2 /\
      forall (dy:vseta) vy, 
                      (val_type env GH T1 n (dy (S n)) vy -> dd (dy n) vy) /\
                      (dd (dy n) vy -> val_type env GH T2 n (dy (S n)) vy)
...
Inductive vtp: list vseta -> list vseta -> ty -> forall n, vset (S n) -> vl -> Prop :=
...
Example ex1: forall G1 GH T n, exists (dd:vset (S (S n))), forall d v, val_type G1 GH T n d v <-> dd d v.
Proof.
...
(* This lemma  establishes that val_type indeed defines a value set (vseta).
   ...
   to [[ TMem T1 T1 ]].
  *)
Example valtp_to_vseta: forall G1 GH T, exists (dd:vseta), forall n d v, val_type G1 GH T n d v <-> dd (S (S n)) d v.
Proof.
  ...
  exists (fun n => match n return vset n with
                     | 0 => fun v => True
                     | 1 => fun d v => True                                       
                     | S (S n0) => (fun d v => val_type G1 GH T n0 d v)
                   end).
  intros.
  split; intros; assumption.
Qed.
...
Definition R_env venv genv tenv :=
  ...
  forall x TX, indexr x tenv = Some TX ->
    (exists (jj:vseta) vx,
       ...
       forall n, vtp genv [] TX n (jj (S n)) vx).
...
Lemma indexr_safe_ex: forall H1 GH G1 TF i,
             R_env H1 GH G1 ->
             indexr i G1 = Some TF ->
             exists d v, indexr i H1 = Some v /\ indexr i GH = Some d /\ forall n, val_type GH [] TF n (d (S n)) v.
Proof.
...
Lemma valtp_extend_aux: forall n T1 vx vf df k H1 G1,
  ...
  (vtp H1 G1 T1 k (df (S k)) vf <-> vtp (vx :: H1) G1 T1 k (df (S k)) vf).
Proof.
  induction n; intros ? ? ? ? ? ? ? S1 C. inversion S1.
  ...
  - destruct vf; try solve [inversion V]. ev.
    ...
    intros.
    assert (forall kx: nat, val_type (H1) G1 T1_1 kx (jj (S kx)) vx0).
    intros. simpl in S1. assert (tsize_flat T1_1 < n). omega. 
  ...
  - destruct vf; try solve by inversion. ev. 
    ...
    assert (forall kx : nat, val_type (vx :: H1) G1 T1_1 kx (jj (S kx)) vx0).
    intros. apply unvv. eapply IHn with (H1 := H1). simpl in S1. omega. assumption. 
    apply vv. auto. 
    specialize (H2 _ _ H4).
    ev. exists x. exists x0. split. assumption. intros. apply unvv. eapply IHn with (H1 := H1).
    unfold open. rewrite <- open_preserves_size. simpl in S1. omega.
    eapply closed_open. simpl. eapply closed_upgrade_free. eassumption. omega.
  ...
  - inversion C. subst. apply vv. rewrite val_type_unfold. destruct vf; try solve by inversion.
    ev. split. simpl. eapply closed_upgrade_freef. eassumption. omega.
        ...
        intros. specialize (H2 dy vy). ev. split.
        intros. apply H2. apply unvv. eapply IHn. simpl in S1. omega. assumption. apply vv. eassumption.
        intros. specialize (H3 H4). apply unvv. eapply IHn with (H1 := H1). simpl in S1. omega.
        assumption. apply vv. assumption.

  - inversion C. subst. apply vv. rewrite val_type_unfold. destruct vf; try solve by inversion.
    ev. split. assumption. split. assumption.
    intros. specialize (H2 dy vy). ev.
    split. intros. apply H2. apply unvv. eapply IHn with (H1:= H1). simpl in S1. omega. assumption.
    apply vv. assumption. intros. specialize (H3 H4). apply unvv. eapply IHn with (H1 := H1). simpl in S1. omega.
  ...
  - inversion C. subst. simpl in *. apply vv. rewrite val_type_unfold.
    assert (closed 1 (length G1) (length H1) T1 /\
        (exists jj : forall x : nat, vset x,
           jj (S k) = df (S k) /\
           (forall n0 : nat,
            val_type H1 (jj :: G1) (open (varH (length G1)) T1) n0 (jj (S n0)) vf))). destruct vf; assumption. clear V. ev.
  ...
  - inversion C. subst. apply vv. rewrite val_type_unfold. 
    assert (closed 1 (length G1) (length (vx :: H1)) T1 /\
        (exists jj : forall x : nat, vset x,
           ...
           (forall n0 : nat,
            val_type (vx :: H1) (jj :: G1) (open (varH (length G1)) T1) n0
              (jj (S n0)) vf))). destruct vf; assumption. clear V. ev.
    assert (closed 1 (length G1) (length H1) T1 /\
    (exists jj : forall x0 : nat, vset x0,
       jj (S k) = df (S k) /\
       (forall n0 : nat,
        val_type H1 (jj :: G1) (open (varH (length G1)) T1) n0 (jj (S n0)) vf))) as Goal. 
    split. assumption. exists x. split. assumption. intros. specialize (H2 n0).
    apply unvv. eapply IHn with (H1 := H1). unfold open. rewrite <- open_preserves_size.
    simpl in S1. omega. eapply closed_open. simpl. eapply closed_upgrade_free. eassumption. omega.
...
Lemma valtp_extend: forall vx vf df k G1 H1 T1,
  val_type H1 G1 T1 k (df (S k)) vf ->
  vtp (vx::H1) G1 T1 k (df (S k)) vf. 
...
Lemma valtp_shrink: forall vx vf df k G1 H1 T1,
  val_type (vx::H1) G1 T1 k (df (S k)) vf ->
  closed 0 (length G1) (length H1) T1 ->                     
  vtp H1 G1 T1 k (df (S k)) vf.
Proof.
...
Lemma valtp_shrinkM: forall vx vf df k H1 GH T1,
  val_type (vx::H1) GH T1 k (df (S k)) vf ->
  closed 0 (length GH) (length H1) T1 ->                     
  vtp H1 GH T1 k (df (S k)) vf.
Proof.
  intros. apply vv in H. eapply valtp_extend_aux. eauto. simpl. assumption.
  eassumption.
Qed.
...
(  
  vtp H (GH1 ++ GH0) T k (df (S k)) vf <-> 
  vtp H (GH1 ++ jj :: GH0) (splice (length GH0) T) k (df (S k)) vf
).
Proof.
  ...
  - destruct vf; try solve by inversion. 
    ...
    intros. assert (forall kx : nat, val_type H (GH1 ++ GH0) T1 kx (jj0 (S kx)) vx).
  ...
  - destruct vf; try solve by inversion. simpl in V.
    ...
    split. assumption. split. assumption. intros.
    assert (forall kx : nat, val_type H (GH1 ++ jj :: GH0) (splice (length GH0) T1) kx (jj0 (S kx)) vx).
  ...
  - inversion C. subst. apply vv. rewrite val_type_unfold. destruct vf; try solve by inversion.
    ...
    split. rewrite app_length. simpl. rewrite E. eapply closed_splice. assumption.
    intros. specialize (H2 dy vy). ev. split. intros. apply H2.
  ...
  - inversion C. subst. apply vv. rewrite val_type_unfold. destruct vf; try solve by inversion. 
    simpl in *. ev. split. assumption. split. assumption. 
        ...
        (exists jj0 : forall x : nat, vset x,
           ...
           (forall n0 : nat,
            val_type H (jj0 :: GH1 ++ GH0)
              (open (varH (length (GH1 ++ GH0))) T) n0 (jj0 (S n0)) vf))).
           ...
           (forall n0 : nat,
            val_type H (jj0 :: GH1 ++ jj :: GH0)
              (open (varH (length (GH1 ++ jj :: GH0)))
                 (splice (length GH0) T)) n0 (jj0 (S n0)) vf))). destruct vf; assumption. clear V. ev.
    assert (closed 1 (length (GH1 ++ GH0)) (length H) T /\
...
Lemma valtp_extendH: forall vf df k H1 GH T1 jj,
  val_type H1 GH T1 k (df (S k)) vf -> 
  vtp H1 (jj::GH) T1 k (df (S k)) vf.
Proof.
...
Lemma valtp_shrinkH: forall vf df k H1 GH T1 jj,
  val_type H1 (jj::GH) T1 k (df (S k)) vf ->
  closed 0 (length GH) (length H1) T1 ->                     
  vtp H1 GH T1 k (df (S k)) vf.
Proof.
  intros. 
  assert (vtp H1 ([] ++ GH) T1 k (df (S k)) vf <-> 
  vtp H1 ([] ++ jj :: GH) (splice (length GH) T1) k (df (S k)) vf).
...
Lemma vtp_subst1: forall venv jj v d k T2,
  val_type venv [jj] (open (varH 0) T2) k (d (S k)) v ->
  closed 0 0 (length venv) T2 ->
  val_type venv [] T2 k (d (S k)) v.
Proof.
...
Lemma vtp_subst2_aux: forall n T venv jj v x d m GH j k,
  ...
  (vtp venv (GH ++ [jj]) (open_rec k (varH 0) (splice 0 T)) m (d (S m)) v <->
   vtp venv GH (open_rec k (varF x) T) m (d (S m)) v).
Proof. admit. (*
  ...
  - inversion Cz. subst.  
    ...
    intros. assert (forall kx : nat, val_type venv0 (GH ++ [jj]) 
      (open_rec k (varH 0) (splice 0 T1)) kx (jj0 (S kx)) vx).
...
Lemma vtp_subst: forall T venv jj v x d k GH,
  ...
  indexr x venv = Some jj ->
  (vtp venv (GH ++ [jj]) (open (varH 0) (splice 0 T)) k (d (S k)) v <->
   vtp venv GH (open (varF x) T) k (d (S k)) v).
...
Lemma vtp_subst2: forall venv jj v x d k  T2,
  ...
  val_type venv [jj] (open (varH 0) T2) k (d (S k)) v ->
  indexr x venv = Some jj ->
  val_type venv [] (open (varF x) T2) k (d (S k)) v.
Proof.
...
Lemma vtp_subst2_general: forall venv jj v x T2 d k,
  closed 1 0 (length venv) T2 ->
  indexr x venv = Some jj ->
  ( vtp venv [jj] (open (varH 0) T2) k (d (S k)) v <->
    vtp venv [] (open (varF x) T2) k (d (S k)) v).
Proof.
  intros. split. 
...
Lemma vtp_subst3: forall venv jj v T2 d k,
  closed 1 0 (length venv) T2 ->
  val_type (jj::venv) [] (open (varF (length venv)) T2) k (d (S k)) v ->
  val_type venv [jj] (open (varH 0) T2) k (d (S k)) v.
Proof.
  intros. apply unvv. assert (splice 0 T2 = T2) as EE. eapply closed_splice_idem. eassumption. omega.
  assert (vtp (jj::venv0) ([] ++ [jj]) (open (varH 0) (splice 0 T2)) k (d (S k)) v).
...
Lemma valty_subst4: forall G T1 jj v d k,
  ...
  (vtp G [jj] (open (varH 0) T1) k (d (S k)) v <->
   vtp (jj :: G) [] (open (varF (length G)) T1) k (d (S k)) v). 
Proof.
  intros. split. 
  Case "->". intros. assert (vtp (jj :: G) [jj] (open (varH 0) T1) k (d (S k)) v).
    ...
    assert (vtp (jj :: G) [] (open (varF (length G)) T1) k (d (S k)) v).
    { eapply vtp_subst2_general. simpl. eapply closed_upgrade_freef. eassumption. omega.
      ...
      assumption. } assumption.
  Case "<-". intros. assert (vtp (jj :: G) [jj] (open (varF (length G)) T1) k (d (S k)) v).
    { eapply valtp_extendH. apply unvv. assumption. }
    assert (vtp (jj :: G) [jj] (open (varH 0) T1) k (d (S k)) v).
...
Lemma valtp_widen_aux: forall G1 GH1 T1 T2,
  ...
  forall (H: list vseta) GH,
    ...
    (forall x TX, indexr x G1 = Some TX ->
                   exists vx jj,
                     indexr x H = Some jj /\
                     (forall n, vtp H GH TX n (jj (S n)) vx)) ->
  ...
  (forall kf (df:vseta) vf, 
     val_type H GH T1 kf (df (S kf)) vf -> vtp H GH T2 kf (df (S kf)) vf).
Proof.
  intros ? ? ? ? stp. 
  ...
  - Case "mem".
    ...
    destruct vf; try destruct b; try solve by inversion; ev.  
...
Lemma approx_vseta: forall k (d:vseta), approx (d (S k)) = d k. Proof. admit. Qed. 
  ...
  - Case "Sel1".
    ...
    specialize (RG _ _ H).
    ev. rewrite H1 in V0.
    assert (x1 (S kf) (df kf) vf). rewrite approx_vseta in V0. destruct vf; eauto. clear V0.

    eapply unvv in H2.
    specialize (IHstp kf x1 x0 H2).
  ...
  - Case "Sel2".
    subst. specialize (IHstp _ _ LG RG LGHX RGHX).
    specialize (RG _ _ H).
    ev. specialize (H2 kf). eapply unvv in H2. 
    ...
    eapply vv. rewrite val_type_unfold. rewrite H1. rewrite approx_vseta. 
 ...
 - Case "Sela1".
           ...
           (exists jj : vseta,
              ...
              (forall n : nat,
               val_type G (jj :: GHX) (open (varH (length GHX)) T1) n 
                        (jj (S n)) vf))). { destruct vf; assumption. }
    clear V0.
  ...
  - Case "Fun".
    subst. 
    rewrite val_type_unfold in V0.
    assert (val_type G GHX (TAll T3 T4) kf (df (S kf)) vf). rewrite val_type_unfold.
    ...
    assert (forall kx, val_type G GHX T1 kx (jj (S kx)) vx) as VX1. {
...
Lemma valtp_widen: forall kf (df:vseta) vf GH H G1 T1 T2,
  val_type GH [] T1 kf (df (S kf)) vf ->
  stp G1 [] T1 T2 ->
  R_env H GH G1 ->
  vtp GH [] T2 kf (df (S kf)) vf.
Proof.
...
Lemma wf_env_extend: forall vx jj G1 R1 H1 T1,
  R_env H1 R1 G1 ->
  (forall n, val_type (jj::R1) [] T1 n (jj (S n)) vx) ->
...
Lemma wf_env_extend0: forall vx jj G1 R1 H1 T1,
  R_env H1 R1 G1 ->
  (forall n, val_type R1 [] T1 n (jj (S n)) vx) ->
  R_env (vx::H1) (jj::R1) (T1::G1).
Proof.
  intros.
  assert (forall n, val_type (jj::R1) [] T1 n (jj (S n)) vx) as V0.
...
Lemma invert_var : forall x tenv T,
  ...
  exists (d: vseta) v, tevaln venv (tvar x) v /\ indexr x venv = Some v /\ indexr x renv = Some d /\ forall k, val_type renv [] T k (d (S k)) v.
Proof.
  ...
  - Case "VarPack".
    ...
    intros. 
    assert (forall n, val_type renv [d] (open (varH 0) T1) n (d (S n)) v). {
...
Theorem full_total_safety : forall e tenv T,
  ...
  exists (d: vseta) v, tevaln venv e v /\ forall k, val_type renv [] T k (d (S k)) v.
Proof.
  intros ? ? ? W. 
  ...
  - Case "unpack". 
    ...
    assert (exists jj : vseta,
              (forall n : nat,
                 val_type renv [jj] (open (varH 0) T1) n (jj (S n)) vx)) as E.
    destruct vx; ev; exists x0; assumption. 
    destruct E as [jj VXH]. 
    assert (forall n, val_type (jj::renv) [] (open (varF (length renv)) T1) n (jj (S n)) vx) as VXF. {
  ...
  - Case "Typ".
    ...
    exists ((fun n => match n return vset n with
                     | 0 => fun v => True
                     | 1 => fun d v => True                                       
                     | S (S n0) => (fun d v => val_type renv [] T1 n0 d v)
                   end)).
    

    
    eexists. split. exists 1. intros. destruct n. inversion H1. simpl. reflexivity.
    rewrite <-(wf_length2 venv0 renv _ WFE) in H.
    intros. rewrite val_type_unfold. simpl. split. assumption. split. assumption. 
    intros. split.
    intros. destruct k. reflexivity. admit. (* downgrade val_type *)
    intros. eapply unvv. destruct k. admit. (* no way to prove from 0 *)
    intros. admit. (* no way to upgrade val_type *)
    

    (* PROBLEM: need to show vtp (S k) -> vtp k /\ vtp k -> vtp (S k) *)

...
(* intersection valtype *)
    | _ , TAnd T1 T2, _ =>
      closed 0 (length GH) (length env) T1 /\ closed 0 (length GH) (length env) T2
      /\ val_type env GH v T1 i /\ val_type env GH v T2 i 
...
Proof. 
  ...
  - (* tand *)
    rewrite val_type_unfold. rewrite val_type_unfold in HV. destruct vy.
...
Proof.
  ...
  - Case "bindx".
    ...
    assert ((exists (vx:vl) (jj : vset),
            ...
            vtp G (jj :: GHX) vf (open (varH (length GHX)) T1) i)) as EV1.
    admit. 
    (* destruct vf; ev; exists x; intros; eapply vv; eapply H4; assumption. *)
    destruct EV1 as [vx [jj [JX [STJ [STB VTX]]]]].

    (* rewrite <-LGHX. rewrite <-LGHX in VT. *)
    subst. 
    assert (if pos i then
      val_type G (jj :: GHX) vf (open (varH (length GH)) T1) i ->
      vtp G (jj :: GHX) vf (open (varH (length GH)) T2) i
    else 
      val_type G (jj :: GHX) vf (open (varH (length GH)) T2) i ->
      vtp G (jj :: GHX) vf (open (varH (length GH)) T1) i) as ST2. {
    ...
    { case_eq (beq_nat x (length GHX)); intros E.
      ...
      + assert (indexr x GH = Some TX) as IX0.
        ...
        assert (vtp G GHX x0 TX nil). specialize (H3 x0 nil). simpl in H3. eapply H3. assumption.
        exists x0. exists x1. split. simpl. rewrite E. eapply H. split. assumption. split. 
        intros. specialize (H3 vy iy). remember (pos iy) as p. destruct p; intros.  
        eapply valtp_extendH. eapply unvv. eapply H3. assumption.
        eapply H3. eapply valtp_shrinkH. eapply unvv. eassumption.
  ...
  - Case "bindx -- reverse".
    ...
    assert ((exists (vx:vl) (jj : vset),
            ...
            vtp G (jj :: GHX) vf (open (varH (length GHX)) T2) i)) as EV1.
    admit. 
    (* destruct vf; ev; exists x; intros; eapply vv; eapply H4; assumption. *)
    destruct EV1 as [vx [jj [JX [STJ [STB VTX]]]]].
    ...
    { case_eq (beq_nat x (length GHX)); intros E.
      + simpl in IX. rewrite LGHX in IX. rewrite E in IX. inversion IX. subst TX.
        ...
        admit.
        (* eapply STJ. eapply unvv. assumption. *)
        (* PROBLEM: have T2, need T1 b/c that's in env *)
        intros. apply (STB _ _ H).
      + assert (indexr x GH = Some TX) as IX0.
        simpl in IX. rewrite LGHX in IX. rewrite E in IX. inversion IX. reflexivity.
        specialize (RGHX _ _ IX0). ev.
  ...
  - Case "Fun".
    ...
    { case_eq (beq_nat x (length GHX)); intros E.
      + simpl in IX. rewrite LGHX in IX. rewrite E in IX. inversion IX. subst TX.
        ...
        assumption. intros. apply (STB _ _ H5).
      + assert (indexr x GH = Some TX) as IX0.
        simpl in IX. rewrite LGHX in IX. rewrite E in IX. inversion IX. reflexivity.
        specialize (RGHX _ _ IX0). ev.
...
Proof.
  ...
  assert (exists (jj:vset),
    ...
    { unfold R_env in R. ev. intros. assert (x < length venv0). apply indexr_max in H6. assumption.
      rewrite H4 in H8. apply indexr_has in H8. ev. specialize (H5 x x0 H8). ev.
      rewrite H6 in H10. inversion H10. subst x2. specialize (H13 v iy H7 vy jy). assumption. }
    intros.
    specialize (valtp_bounds _ _ _ _ H3 RR). intros VB.
